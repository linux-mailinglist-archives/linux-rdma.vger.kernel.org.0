...
Received: from vger.kernel.org (vger.kernel.org [209.132.180.67])
	by mail.lfdr.de (Postfix) with ESMTP id ECBB92A97C
	for <lists+linux-rdma@lfdr.de>; Sun, 26 May 2019 13:43:02 +0200 (CEST)
Received: (majordomo@vger.kernel.org) by vger.kernel.org via listexpand
        id S1727638AbfEZLnC (ORCPT <rfc822;lists+linux-rdma@lfdr.de>);
        Sun, 26 May 2019 07:43:02 -0400
Received: from mx0b-001b2d01.pphosted.com ([148.163.158.5]:50548 "EHLO
        mx0a-001b2d01.pphosted.com" rhost-flags-OK-OK-OK-FAIL)
        by vger.kernel.org with ESMTP id S1727731AbfEZLnC (ORCPT
        <rfc822;linux-rdma@vger.kernel.org>);
        Sun, 26 May 2019 07:43:02 -0400
Received: from pps.filterd (m0098414.ppops.net [127.0.0.1])
        by mx0b-001b2d01.pphosted.com (8.16.0.27/8.16.0.27) with SMTP id x4QBgpFr114384
        for <linux-rdma@vger.kernel.org>; Sun, 26 May 2019 07:42:53 -0400
Received: from e06smtp03.uk.ibm.com (e06smtp03.uk.ibm.com [195.75.94.99])
        by mx0b-001b2d01.pphosted.com with ESMTP id 2sqk0gapt7-1
...
Received: from b06cxnps3074.portsmouth.uk.ibm.com (9.149.109.194)
        ...
        Sun, 26 May 2019 12:42:28 +0100
Received: from b06wcsmtp001.portsmouth.uk.ibm.com (b06wcsmtp001.portsmouth.uk.ibm.com [9.149.105.160])
        by b06cxnps3074.portsmouth.uk.ibm.com (8.14.9/8.14.9/NCO v10.0) with ESMTP id x4QBgRCf61604034
        (version=TLSv1/SSLv3 cipher=DHE-RSA-AES256-GCM-SHA384 bits=256 verify=OK);
        Sun, 26 May 2019 11:42:27 GMT
Received: from b06wcsmtp001.portsmouth.uk.ibm.com (unknown [127.0.0.1])
        by IMSVA (Postfix) with ESMTP id DE300A4054;
        Sun, 26 May 2019 11:42:26 +0000 (GMT)
Received: from b06wcsmtp001.portsmouth.uk.ibm.com (unknown [127.0.0.1])
        by IMSVA (Postfix) with ESMTP id AB0D1A405F;
        Sun, 26 May 2019 11:42:26 +0000 (GMT)
...
Subject: [PATCH for-next v1 05/12] SIW application interface
Date:   Sun, 26 May 2019 13:41:49 +0200
X-Mailer: git-send-email 2.17.2
In-Reply-To: <20190526114156.6827-1-bmt@zurich.ibm.com>
References: <20190526114156.6827-1-bmt@zurich.ibm.com>
X-TM-AS-GCONF: 00
x-cbid: 19052611-0012-0000-0000-0000031F8C01
X-IBM-AV-DETECTION: SAVI=unused REMOTE=unused XFE=unused
x-cbparentid: 19052611-0013-0000-0000-000021584C96
Message-Id: <20190526114156.6827-6-bmt@zurich.ibm.com>
...
+					  base_ucontext);
+
+	siw_dbg_cq(cq, "free CQ\n");
+
+	siw_cq_flush(cq);
+
+	if (ctx && cq->xa_cq_index != SIW_INVAL_UOBJ_KEY)
+		siw_delete_uobj(ctx, cq->xa_cq_index);
+
+	atomic_dec(&cq->sdev->num_cq);
+
+	if (cq->queue)
+		vfree(cq->queue);
+
...
+		siw_dbg(base_dev, "too many cq's\n");
+		rv = -ENOMEM;
+		goto err_out;
+	}
+	if (size < 1 || size > sdev->attrs.max_cqe) {
+		siw_dbg(base_dev, "cq size error: %d\n", size);
+		rv = -EINVAL;
+		goto err_out;
+	}
...
+
+err_out:
+	siw_dbg(base_dev, "cq creation failed: %d", rv);
+
+	if (cq && cq->queue) {
+		struct siw_ucontext *ctx =
+			rdma_udata_to_drv_context(udata, struct siw_ucontext,
...
+			siw_delete_uobj(ctx, srq->xa_srq_index);
+		vfree(srq->recvq);
+	}
...
new file mode 100644
index 000000000000..8b00f899a9bf
--- /dev/null
...
+}
+
+extern int siw_alloc_ucontext(struct ib_ucontext *base_ctx,
+			      struct ib_udata *udata);
+extern void siw_dealloc_ucontext(struct ib_ucontext *base_ctx);
+extern int siw_query_port(struct ib_device *base_dev, u8 port,
+			  struct ib_port_attr *attr);
+extern int siw_get_port_immutable(struct ib_device *base_dev, u8 port,
+				  struct ib_port_immutable *port_immutable);
+extern int siw_query_device(struct ib_device *base_dev,
+			    struct ib_device_attr *attr,
+			    struct ib_udata *udata);
+extern struct ib_cq *siw_create_cq(struct ib_device *base_dev,
+				   const struct ib_cq_init_attr *attr,
+				   struct ib_udata *udata);
+extern int siw_query_port(struct ib_device *base_dev, u8 port,
+			  struct ib_port_attr *attr);
+extern int siw_query_pkey(struct ib_device *base_dev, u8 port, u16 idx,
+			  u16 *pkey);
+extern int siw_query_gid(struct ib_device *base_dev, u8 port, int idx,
+			 union ib_gid *gid);
+extern int siw_alloc_pd(struct ib_pd *base_pd, struct ib_udata *udata);
+extern void siw_dealloc_pd(struct ib_pd *base_pd, struct ib_udata *udata);
+extern struct ib_qp *siw_create_qp(struct ib_pd *base_pd,
+				   struct ib_qp_init_attr *attr,
+				   struct ib_udata *udata);
+extern int siw_query_qp(struct ib_qp *base_qp, struct ib_qp_attr *qp_attr,
+			int qp_attr_mask, struct ib_qp_init_attr *qp_init_attr);
+extern int siw_verbs_modify_qp(struct ib_qp *base_qp, struct ib_qp_attr *attr,
+			       int attr_mask, struct ib_udata *udata);
+extern int siw_destroy_qp(struct ib_qp *base_qp, struct ib_udata *udata);
+extern int siw_post_send(struct ib_qp *base_qp, const struct ib_send_wr *wr,
+			 const struct ib_send_wr **bad_wr);
+extern int siw_post_receive(struct ib_qp *base_qp, const struct ib_recv_wr *wr,
+			    const struct ib_recv_wr **bad_wr);
+extern int siw_destroy_cq(struct ib_cq *base_cq, struct ib_udata *udata);
+extern int siw_poll_cq(struct ib_cq *base_cq, int num_entries,
+		       struct ib_wc *wc);
+extern int siw_req_notify_cq(struct ib_cq *base_cq,
+			     enum ib_cq_notify_flags flags);
+extern struct ib_mr *siw_reg_user_mr(struct ib_pd *base_pd, u64 start,
+				     u64 len, u64 rnic_va, int rights,
+				     struct ib_udata *udata);
+extern struct ib_mr *siw_alloc_mr(struct ib_pd *base_pd,
+				  enum ib_mr_type mr_type, u32 max_sge,
+				  struct ib_udata *udata);
+extern struct ib_mr *siw_get_dma_mr(struct ib_pd *base_pd, int rights);
+extern int siw_map_mr_sg(struct ib_mr *base_mr, struct scatterlist *sl,
+			 int num_sle, unsigned int *sg_off);
+extern int siw_dereg_mr(struct ib_mr *base_mr, struct ib_udata *udata);
+extern int siw_create_srq(struct ib_srq *base_srq,
+			  struct ib_srq_init_attr *attr,
+			  struct ib_udata *udata);
+extern int siw_modify_srq(struct ib_srq *base_srq, struct ib_srq_attr *attr,
+			  enum ib_srq_attr_mask mask, struct ib_udata *udata);
+extern int siw_query_srq(struct ib_srq *base_srq, struct ib_srq_attr *attr);
+extern void siw_destroy_srq(struct ib_srq *base_srq, struct ib_udata *udata);
+extern int siw_post_srq_recv(struct ib_srq *base_srq,
+			     const struct ib_recv_wr *wr,
+			     const struct ib_recv_wr **bad_wr);
+extern int siw_mmap(struct ib_ucontext *ctx, struct vm_area_struct *vma);
...
new file mode 100644
index 000000000000..b3d8ee67ec0a
--- /dev/null
