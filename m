...
Received: from vger.kernel.org (vger.kernel.org [23.128.96.18])
	by mail.lfdr.de (Postfix) with ESMTP id E07DD301179
	for <lists+linux-rdma@lfdr.de>; Sat, 23 Jan 2021 01:15:37 +0100 (CET)
Received: (majordomo@vger.kernel.org) by vger.kernel.org via listexpand
        id S1726352AbhAWAOu (ORCPT <rfc822;lists+linux-rdma@lfdr.de>);
        Fri, 22 Jan 2021 19:14:50 -0500
Received: from mga05.intel.com ([192.55.52.43]:55261 "EHLO mga05.intel.com"
        rhost-flags-OK-OK-OK-OK) by vger.kernel.org with ESMTP
        id S1726375AbhAVXwF (ORCPT <rfc822;linux-rdma@vger.kernel.org>);
        Fri, 22 Jan 2021 18:52:05 -0500
IronPort-SDR: D0Od6rMl1uR3E3FJM/c7qDByHX3LhDu0UvdJp62m2/+0W6N/+HJFZ2/xrkNCtKbwVisEqdlk6B
 nWZPUmZJy/Ng==
X-IronPort-AV: E=McAfee;i="6000,8403,9872"; a="264346876"
X-IronPort-AV: E=Sophos;i="5.79,368,1602572400"; 
   d="scan'208";a="264346876"
Received: from fmsmga005.fm.intel.com ([10.253.24.32])
  by fmsmga105.fm.intel.com with ESMTP/TLS/ECDHE-RSA-AES256-GCM-SHA384; 22 Jan 2021 15:50:15 -0800
IronPort-SDR: NqU00EhVMDhWW2SxkYhtnSJQ0pnOLwbXUk1RqYNNYsy1mexMizs5JttoekBhb/W5RY/ADMYWVZ
 pQF4i0fhRyfw==
X-IronPort-AV: E=Sophos;i="5.79,368,1602572400"; 
   d="scan'208";a="574869455"
Received: from ssaleem-mobl.amr.corp.intel.com ([10.251.4.95])
  by fmsmga005-auth.fm.intel.com with ESMTP/TLS/ECDHE-RSA-AES256-GCM-SHA384; 22 Jan 2021 15:50:12 -0800
...
Subject: [PATCH 08/22] RDMA/irdma: Implement device initialization definitions
Date:   Fri, 22 Jan 2021 17:48:13 -0600
Message-Id: <20210122234827.1353-9-shiraz.saleem@intel.com>
...
---
 ...
 5 files changed, 3305 insertions(+)
...
+			break;
+		if (status) {
+			irdma_dbg(dev, "ERR: puda status = %d\n", status);
+			break;
+		}
+		if (compl_error) {
+			irdma_dbg(dev, "ERR: puda compl_err  =0x%x\n",
+				  compl_error);
+			break;
+		}
+	} while (1);
...
+}
+
+static void irdma_set_roce_flush_error_code(struct irdma_sc_qp *qp, struct irdma_aeqe_info *info)
+{
...
+			  info->tcp_state, info->iwarp_state, info->ae_src);
+
+		if (info->qp) {
...
+					continue;
+				}
+				irdma_dbg(dev, "AEQ: qp_id %d is already freed\n", info->qp_cq_id);
+				continue;
+			}
...
+									    ctx_info);
+				}
+				irdma_set_roce_flush_error_code(qp, info);
+				irdma_cm_disconn(iwqp);
+				break;
+			}
...
+ * @rf: RDMA PCI function
+ *
+ * Allocate iwdev msix table and copy the peer_info msix info to the table
+ * Return 0 if successful, otherwise return error
+ */
+static enum irdma_status_code irdma_save_msix_info(struct irdma_pci_f *rf)
+{
+	struct irdma_priv_peer_info *peer_info = &rf->priv_peer_info;
...
+	u32 size;
+
+	if (!peer_info->msix_count)
+		return IRDMA_ERR_NO_INTR;
+
+	rf->msix_count = peer_info->msix_count;
...
+	if (status)
+		irdma_dbg(dev, "ERR: Destroy CQP failed %d\n", status);
+
+	irdma_cleanup_pending_cqp_op(rf);
+	dma_free_coherent(ihw_to_dev(dev->hw), cqp->sq.size, cqp->sq.va,
+			  cqp->sq.pa);
+	cqp->sq.va = NULL;
...
+	u64 *pg_arr = (u64 *)(uintptr_t)aeq->palloc.level1.addr;
+
...
+	if (status)
+		irdma_dbg(dev, "ERR: Destroy AEQ failed %d\n", status);
+
+exit:
+	if (aeq->virtual_map) {
+		irdma_destroy_virt_aeq(rf);
+	} else {
+		dma_free_coherent(ihw_to_dev(dev->hw), aeq->mem.size,
...
+	if (status) {
+		irdma_dbg(dev, "ERR: CEQ destroy command failed %d\n", status);
+		goto exit;
+	}
+
+	status = dev->ceq_ops->cceq_destroy_done(&iwceq->sc_ceq);
+	if (status)
+		irdma_dbg(dev, "ERR: CEQ destroy completion failed %d\n",
+			  status);
+exit:
+	dma_free_coherent(ihw_to_dev(dev->hw), iwceq->mem.size, iwceq->mem.va,
+			  iwceq->mem.pa);
+	iwceq->mem.va = NULL;
+}
+
...
+		dma_free_coherent(ihw_to_dev(rf->sc_dev.hw), iwceq->mem.size,
...
+	if (status)
+		irdma_dbg(dev, "ERR: CCQ destroy failed %d\n", status);
+	dma_free_coherent(ihw_to_dev(dev->hw), ccq->mem_cq.size,
...
+		irdma_dbg(dev, "ERR: del HMC obj of type %d failed\n", obj_type);
+}
+
+/**
...
+			     IRDMA_CQP_ALIGNMENT);
+	cqp->sq.va = dma_alloc_coherent(ihw_to_dev(dev->hw), cqp->sq.size,
...
+	if (status) {
+		irdma_dbg(dev, "ERR: cqp init status %d\n", status);
+		goto exit;
+	}
+
...
+				 IRDMA_CQ0_ALIGNMENT);
+	ccq->mem_cq.va = dma_alloc_coherent(ihw_to_dev(dev->hw),
...
+	if (status) {
+		irdma_dbg(&rf->sc_dev, "ERR: ceq irq config fail\n");
...
+	}
+	if (ret) {
+		irdma_dbg(&rf->sc_dev, "ERR: aeq irq config fail\n");
+		return IRDMA_ERR_CFG;
+	}
+
...
+				IRDMA_CEQ_ALIGNMENT);
+	iwceq->mem.va = dma_alloc_coherent(ihw_to_dev(dev->hw),
...
+
+	if (status) {
+		dma_free_coherent(ihw_to_dev(dev->hw), iwceq->mem.size,
+				  iwceq->mem.va, iwceq->mem.pa);
+		iwceq->mem.va = NULL;
+	}
+
...
+	if (status) {
+		irdma_dbg(&rf->sc_dev, "ERR: create ceq status = %d\n",
+			  status);
+		goto exit;
+	}
+
...
+	}
+
+	pg_arr = (u64 *)(uintptr_t)aeq->palloc.level1.addr;
...
+			      IRDMA_AEQ_ALIGNMENT);
+	aeq->mem.va = dma_alloc_coherent(ihw_to_dev(dev->hw), aeq->mem.size,
+					 &aeq->mem.pa,
...
+	if (status)
+		irdma_dbg(&iwdev->rf->sc_dev, "ERR: ilq create fail\n");
+
+	return status;
+}
+
...
+	status = irdma_puda_create_rsrc(&iwdev->vsi, &info);
+	if (status)
+		irdma_dbg(&iwdev->rf->sc_dev, "ERR: ieq create fail\n");
+
+	return status;
+}
+
...
+	kfree(rf->mem_rsrc);
+	rf->mem_rsrc = NULL;
+	dma_free_coherent(ihw_to_dev(&rf->hw), rf->obj_mem.size,
...
+ * @rf: RDMA PCI function
+ * @peer_info: lan device information
+ *
...
+						   struct irdma_priv_peer_info *peer_info)
+{
+	enum irdma_status_code status;
+	struct irdma_sc_dev *dev = &rf->sc_dev;
...
+	info.hmc_fn_id = (u8)peer_info->fn_num;
+	info.is_pf = !peer_info->ftype;
+	info.privileged = !peer_info->ftype;
+	info.hw = &rf->hw;
...
+{
+	irdma_dbg(&iwdev->rf->sc_dev, "INIT: state = %d\n", iwdev->init_state);
+
...
+ * using the peer_info and client information
+ * Return 0 if successful, otherwise return error
+ */
...
+		return status;
+
+	rf->hw.pcidev = rf->pcidev;
+	rf->obj_mem.size = ALIGN(8192, IRDMA_HW_PAGE_SIZE);
+	rf->obj_mem.va = dma_alloc_coherent(ihw_to_dev(&rf->hw),
...
+	status = irdma_initialize_dev(rf, peer_info);
+	if (status) {
+		kfree(rf->iw_msixtbl);
+		dma_free_coherent(ihw_to_dev(&rf->hw), rf->obj_mem.size,
+				  rf->obj_mem.va, rf->obj_mem.pa);
...
+	vsi_info.vm_vf_type = rf->priv_peer_info.ftype ? IRDMA_VF_TYPE : IRDMA_PF_TYPE;
+	vsi_info.dev = dev;
...
+	} while (0);
+
+	dev_err(idev_to_dev(dev), "VSI open FAIL status = %d last cmpl = %d\n",
...
+	num_ahs = rf->sc_dev.hmc_info->hmc_obj[IRDMA_HMC_IW_FSIAV].cnt;
...
+	irdma_dbg(iwdev_to_idev(iwdev), "DEV: %s: port=0x%04x\n",
...
+#define I40E_COMMIT_FPM_CQCNT_MASK	(0x3ffffULL << I40E_COMMIT_FPM_CQCNT_S)
+
...
+
+#define I40E_CQPSQ_STAG_PDID_SHIFT         48ULL
