...
Received: from vger.kernel.org (vger.kernel.org [23.128.96.18])
	by mail.lfdr.de (Postfix) with ESMTP id A9E5F2F1997
	for <lists+linux-rdma@lfdr.de>; Mon, 11 Jan 2021 16:25:03 +0100 (CET)
Received: (majordomo@vger.kernel.org) by vger.kernel.org via listexpand
        id S1728506AbhAKPZC (ORCPT <rfc822;lists+linux-rdma@lfdr.de>);
        Mon, 11 Jan 2021 10:25:02 -0500
Received: from mga04.intel.com ([192.55.52.120]:42991 "EHLO mga04.intel.com"
        rhost-flags-OK-OK-OK-OK) by vger.kernel.org with ESMTP
        id S1726890AbhAKPZC (ORCPT <rfc822;linux-rdma@vger.kernel.org>);
        Mon, 11 Jan 2021 10:25:02 -0500
IronPort-SDR: SUyv83E/F33q+5+LJYhikEHzO/U82p4xv9eeZCfxJzc0tDsX8QjGSETu3MNDakWeQpCHJtUbCU
 Vx8lM00AuaJQ==
X-IronPort-AV: E=McAfee;i="6000,8403,9860"; a="175301480"
X-IronPort-AV: E=Sophos;i="5.79,338,1602572400"; 
   d="scan'208";a="175301480"
Received: from fmsmga002.fm.intel.com ([10.253.24.26])
  by fmsmga104.fm.intel.com with ESMTP/TLS/ECDHE-RSA-AES256-GCM-SHA384; 11 Jan 2021 07:24:21 -0800
IronPort-SDR: CtnDuOUkoGWV0CG8EHCC+rlcyS1GKN1xTABaB72+I8s02JfTaX35/NjFuJpaIAR+OBXaLIha5z
 iI8h0RgpWOfA==
X-ExtLoop1: 1
X-IronPort-AV: E=Sophos;i="5.79,338,1602572400"; 
   d="scan'208";a="399830097"
...
x-forefront-antispam-report: CIP:255.255.255.255;CTRY:;LANG:en;SCL:1;SRV:;IPV:NLI;SFV:NSPM;H:MW3PR11MB4555.namprd11.prod.outlook.com;PTR:;CAT:NONE;SFS:(4636009)(396003)(136003)(366004)(376002)(346002)(39860400002)(76116006)(966005)(4326008)(66946007)(7696005)(64756008)(71200400001)(53546011)(107886003)(86362001)(478600001)(52536014)(5660300002)(66446008)(66476007)(30864003)(9686003)(66556008)(110136005)(55016002)(83380400001)(8936002)(54906003)(26005)(8676002)(33656002)(186003)(6506007)(2906002)(316002);DIR:OUT;SFP:1102;
...
X-MS-Exchange-CrossTenant-originalarrivaltime: 11 Jan 2021 15:24:18.4365
 (UTC)
...
>   'struct ib_umem *'
> * Move the check of on demand paging support to mlx5_ib_reg_user_mr_dmabu=
f()
...
>   https://cgit.freedesktop.org/drm/drm-misc/commit/?id=3D476b485be03c
...
> * Fix formating issue -- unnecessary alignment of '=3D'
...
>   https://cgit.freedesktop.org/drm/drm-misc/commit/?id=3Dac80cd17a615
...
>=20
> When enabled, an RDMA capable NIC can perform peer-to-peer transactions o=
ver PCIe to access the local memory located on another
> device. This can often lead to better performance than using a system mem=
ory buffer for RDMA and copying data between the buffer and
> device memory.
>=20
> Current kernel RDMA stack uses get_user_pages() to pin the physical pages=
 backing the user buffer and uses dma_map_sg_attrs() to get the
> dma addresses for memory access. This usually doesn't work for peer devic=
e memory due to the lack of associated page structures.
>=20
> Several mechanisms exist today to facilitate device memory access.
>=20
> ZONE_DEVICE is a new zone for device memory in the memory management subs=
ystem. It allows pages from device memory being
> described with specialized page structures, but what can be done with the=
se page structures may be different from system memory.
> ZONE_DEVICE is further specialized into multiple memory types, such as on=
e type for PCI p2pmem/p2pdma and one type for HMM.
>=20
> PCI p2pmem/p2pdma uses ZONE_DEVICE to represent device memory residing in=
 a PCI BAR and provides a set of calls to publish, discover,
> allocate, and map such memory for peer-to-peer transactions. One feature =
of the API is that the buffer is allocated by the side that does the
> DMA transfer. This works well with the storage usage case, but is awkward=
 with GPU-NIC communication, where typically the buffer is
> allocated by the GPU driver rather than the NIC driver.
>=20
> Heterogeneous Memory Management (HMM) utilizes mmu_interval_notifier and =
ZONE_DEVICE to support shared virtual address space and
> page migration between system memory and device memory. HMM doesn't suppo=
rt pinning device memory because pages located on
> device must be able to migrate to system memory when accessed by CPU. Pee=
r-to-peer access is currently not supported by HMM.
>=20
> Dma-buf is a standard mechanism for sharing buffers among different devic=
e drivers. The buffer to be shared is exported by the owning
> driver and imported by the driver that wants to use it. The exporter prov=
ides a set of ops that the importer can call to pin and map the
> buffer. In addition, a file descriptor can be associated with a dma- buf =
object as the handle that can be passed to user space.
>=20
> This patch series adds dma-buf importer role to the RDMA driver in attemp=
t to support RDMA using device memory such as GPU VRAM.
> Dma-buf is chosen for a few reasons: first, the API is relatively simple =
and allows a lot of flexibility in implementing the buffer manipulation
> ops.
> Second, it doesn't require page structure. Third, dma-buf is already supp=
orted in many GPU drivers. However, we are aware that existing
...
> True peer-to-peer access is only possible using dynamic attach, which req=
uires on-demand paging support from the NIC to work. For this
> reason, this series only works with ODP capable NICs.
>=20
> This series consists of four patches. The first patch adds the common cod=
e for importing dma-buf from a file descriptor and mapping the
> dma-buf pages. Patch 2 add the new driver method reg_user_mr_dmabuf().
> Patch 3 adds a new uverbs command for registering dma-buf based memory re=
gion. Patch 4 adds dma-buf support to the mlx5 driver.
>=20
...
>  13 files changed, 573 insertions(+), 14 deletions(-)  create mode 100644=
