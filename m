...
Received: from vger.kernel.org (vger.kernel.org [23.128.96.18])
	by mail.lfdr.de (Postfix) with ESMTP id E8416305E5C
	for <lists+linux-rdma@lfdr.de>; Wed, 27 Jan 2021 15:33:26 +0100 (CET)
Received: (majordomo@vger.kernel.org) by vger.kernel.org via listexpand
        id S233212AbhA0OdG (ORCPT <rfc822;lists+linux-rdma@lfdr.de>);
        Wed, 27 Jan 2021 09:33:06 -0500
Received: from mail.kernel.org ([198.145.29.99]:44354 "EHLO mail.kernel.org"
        rhost-flags-OK-OK-OK-OK) by vger.kernel.org with ESMTP
        id S233734AbhA0Obr (ORCPT <rfc822;linux-rdma@vger.kernel.org>);
        Wed, 27 Jan 2021 09:31:47 -0500
Received: by mail.kernel.org (Postfix) with ESMTPSA id AB989207C7;
        Wed, 27 Jan 2021 14:31:04 +0000 (UTC)
DKIM-Signature: v=1; a=rsa-sha256; c=relaxed/simple; d=kernel.org;
        s=k20201202; t=1611757865;
        bh=GH+k031Wkfu76iuD0loT7DLISBFIlz3yXbigT/G+aPA=;
        h=From:To:Cc:Subject:Date:From;
        b=
...
Subject: [PATCH mlx5-next] RDMA/mlx5: Cleanup the synchronize_srcu() from the ODP flow
Date:   Wed, 27 Jan 2021 16:30:51 +0200
Message-Id: <20210127143051.1873866-1-leon@kernel.org>
...
---
 ...
 drivers/infiniband/hw/mlx5/odp.c             | 216 ++++++++-----------
...
index d15943b00c61..2f38d00bc2af 100644
--- a/drivers/infiniband/hw/mlx5/main.c
+++ b/drivers/infiniband/hw/mlx5/main.c
@@ -3953,8 +3953,8 @@ static void mlx5_ib_stage_init_cleanup(struct mlx5_ib_dev *dev)
 {
...
@@ -4005,10 +4005,6 @@ static int mlx5_ib_stage_init_init(struct mlx5_ib_dev *dev)
...
@@ -4018,6 +4014,7 @@ static int mlx5_ib_stage_init_init(struct mlx5_ib_dev *dev)
...
index e77b1db73893..f4b82daf1e22 100644
...
+
 	err = mlx5_ib_update_xlt(mr, 0,
 				 ...
 				 PAGE_SHIFT,
@@ -502,27 +483,32 @@ static struct mlx5_ib_mr *implicit_get_child_mr(struct mlx5_ib_mr *imr,
 		goto out_mr;
 	}
...
+	xa_unlock(&imr->implicit_children);
 ...
 out_umem:
@@ -561,8 +547,6 @@ struct mlx5_ib_mr *mlx5_ib_alloc_implicit_mr(struct mlx5_ib_pd *pd,
...
@@ -574,6 +558,7 @@ struct mlx5_ib_mr *mlx5_ib_alloc_implicit_mr(struct mlx5_ib_pd *pd,
 	if (err)
 		goto out_mr;
...
+	refcount_set(&imr->mmkey.usecount, 1);
 	...
 	if (err)
@@ -593,51 +578,24 @@ void mlx5_ib_free_implicit_mr(struct mlx5_ib_mr *imr)
 {
 	struct ib_umem_odp *odp_imr = to_ib_umem_odp(imr->umem);
...
@@ -645,8 +603,10 @@ void mlx5_ib_free_implicit_mr(struct mlx5_ib_mr *imr)
 	 */
...
@@ -665,9 +625,7 @@ void mlx5_ib_fence_odp_mr(struct mlx5_ib_mr *mr)
...
@@ -686,10 +644,7 @@ void mlx5_ib_fence_dmabuf_mr(struct mlx5_ib_mr *mr)
...
@@ -780,8 +735,10 @@ static int pagefault_implicit_mr(struct mlx5_ib_mr *imr,
 		struct mlx5_ib_mr *mtt;
 		u64 len;
...
+			xa_unlock(&imr->implicit_children);
 			...
 			if (IS_ERR(mtt)) {
 				ret = PTR_ERR(mtt);
@@ -789,6 +746,9 @@ static int pagefault_implicit_mr(struct mlx5_ib_mr *imr,
 			}
...
@@ -797,6 +757,9 @@ static int pagefault_implicit_mr(struct mlx5_ib_mr *imr,
...
+
 		if (ret < 0)
 			goto out;
 		user_va += len;
@@ -888,7 +851,6 @@ static int pagefault_mr(struct mlx5_ib_mr *mr, u64 io_virt, size_t bcnt,
 {
 	struct ib_umem_odp *odp = to_ib_umem_odp(mr->umem);
...
@@ -980,7 +942,7 @@ static int pagefault_single_data_segment(struct mlx5_ib_dev *dev,
 					 u32 *bytes_committed,
 					 u32 *bytes_mapped)
 {
...
+	int npages = 0, ret, i, outlen, cur_outlen = 0, depth = 0;
 	...
 	struct mlx5_core_mkey *mmkey;
 	struct mlx5_ib_mr *mr;
@@ -989,14 +951,14 @@ static int pagefault_single_data_segment(struct mlx5_ib_dev *dev,
 	size_t offset;
 	int ndescs;
...
@@ -1009,12 +971,15 @@ static int pagefault_single_data_segment(struct mlx5_ib_dev *dev,
 		 * faulted.
 		 */
 		ret = 0;
-		goto srcu_unlock;
...
+		goto end;
 	...
 	switch (mmkey->type) {
@@ -1023,7 +988,7 @@ static int pagefault_single_data_segment(struct mlx5_ib_dev *dev,
...
@@ -1038,7 +1003,7 @@ static int pagefault_single_data_segment(struct mlx5_ib_dev *dev,
...
@@ -1049,7 +1014,7 @@ static int pagefault_single_data_segment(struct mlx5_ib_dev *dev,
...
@@ -1059,7 +1024,7 @@ static int pagefault_single_data_segment(struct mlx5_ib_dev *dev,
...
@@ -1073,7 +1038,7 @@ static int pagefault_single_data_segment(struct mlx5_ib_dev *dev,
...
@@ -1092,7 +1057,7 @@ static int pagefault_single_data_segment(struct mlx5_ib_dev *dev,
 	default:
...
+		goto end;
 	...
 	if (head) {
@@ -1105,10 +1070,13 @@ static int pagefault_single_data_segment(struct mlx5_ib_dev *dev,
 		depth = frame->depth;
 		kfree(frame);
...
+		mlx5r_deref_odp_mkey(mmkey);
 	while (head) {
 		frame = head;
 		head = frame->next;
@@ -1116,7 +1084,6 @@ static int pagefault_single_data_segment(struct mlx5_ib_dev *dev,
 	}
 	kfree(out);
...
@@ -1824,8 +1791,8 @@ static void destroy_prefetch_work(struct prefetch_mr_work *work)
 	u32 i;
...
@@ -1835,24 +1802,30 @@ get_prefetchable_mr(struct ib_pd *pd, enum ib_uverbs_advise_mr_advice advice,
 {
...
@@ -1862,15 +1835,12 @@ static void mlx5_ib_prefetch_mr_work(struct work_struct *w)
...
@@ -1879,7 +1849,6 @@ static void mlx5_ib_prefetch_mr_work(struct work_struct *w)
 			continue;
...
@@ -1903,9 +1872,6 @@ static bool init_prefetch_work(struct ib_pd *pd,
 			work->num_sge = i;
 			return false;
 		}
-
...
-		atomic_inc(&work->frags[i].mr->num_deferred_work);
 	...
 	return true;
@@ -1916,42 +1882,35 @@ static int mlx5_ib_prefetch_sg_list(struct ib_pd *pd,
...
@@ -1967,13 +1926,10 @@ int mlx5_ib_advise_mr_prefetch(struct ib_pd *pd,
 	if (!work)
 		return -ENOMEM;
