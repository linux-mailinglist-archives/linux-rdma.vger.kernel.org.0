...
Received: from vger.kernel.org (vger.kernel.org [209.132.180.67])
	by mail.lfdr.de (Postfix) with ESMTP id 649C51250BA
	for <lists+linux-rdma@lfdr.de>; Wed, 18 Dec 2019 19:37:23 +0100 (CET)
Received: (majordomo@vger.kernel.org) by vger.kernel.org via listexpand
        id S1726824AbfLRShW (ORCPT <rfc822;lists+linux-rdma@lfdr.de>);
        Wed, 18 Dec 2019 13:37:22 -0500
Received: from mail-eopbgr20078.outbound.protection.outlook.com ([40.107.2.78]:60133
        "EHLO EUR02-VE1-obe.outbound.protection.outlook.com"
        rhost-flags-OK-OK-OK-FAIL) by vger.kernel.org with ESMTP
        id S1726698AbfLRShW (ORCPT <rfc822;linux-rdma@vger.kernel.org>);
        Wed, 18 Dec 2019 13:37:22 -0500
...
X-MS-Exchange-CrossTenant-originalarrivaltime: 18 Dec 2019 18:37:09.6290
 (UTC)
 ...
 *ops,
@@ -364,7 +364,7 @@ static inline bool mmu_interval_check_retry(struct mmu_=
interval_notifier *mni,
 	return READ_ONCE(mni->invalidate_seq) !=3D seq;
 }
=20
...
+extern void __mmu_notifier_subscriptions_destroy(struct mm_struct *mm);
 ...
 extern int __mmu_notifier_clear_flush_young(struct mm_struct *mm,
 					  unsigned long start,
@@ -480,15 +480,15 @@ static inline void mmu_notifier_invalidate_range(stru=
ct mm_struct *mm,
...
+	mm->notifier_subscriptions =3D NULL;
 }
=20
...
=20
=20
@@ -692,11 +692,11 @@ static inline void mmu_notifier_invalidate_range(stru=
ct mm_struct *mm,
 {
 }
=20
...
+	mmu_notifier_subscriptions_destroy(mm);
 	...
 	free_mm(mm);
@@ -1025,7 +1025,7 @@ static struct mm_struct *mm_init(struct mm_struct *mm=
...
--- a/mm/mmu_notifier.c
+++ b/mm/mmu_notifier.c
@@ -29,12 +29,12 @@ struct lockdep_map __mmu_notifier_invalidate_range_star=
t_map =3D {
 #endif
=20
 /*
...
+ * Operations on notifier_subscriptions->invalidate_seq (under spinlock):
  *    seq |=3D 1  # Begin writing
  *    seq++     # Release the writing state
  *    seq & 1   # True if a writer exists
@@ -83,32 +83,33 @@ struct mmu_notifier_mm {
  * The later state avoids some expensive work on inv_end in the common cas=
e of
...
+	node =3D interval_tree_iter_first(&subscriptions->itree, range->start,
 					range->end - 1);
 	if (node) {
-		mmn_mm->invalidate_seq |=3D 1;
+		subscriptions->invalidate_seq |=3D 1;
 		res =3D container_of(node, struct mmu_interval_notifier,
 				   interval_tree);
 	}
=20
-	*seq =3D mmn_mm->invalidate_seq;
-	spin_unlock(&mmn_mm->lock);
+	*seq =3D subscriptions->invalidate_seq;
...
+static void mn_itree_inv_end(struct mmu_notifier_subscriptions *subscripti=
ons)
 {
...
@@ -146,19 +147,19 @@ static void mn_itree_inv_end(struct mmu_notifier_mm *=
mmn_mm)
...
=20
 /**
@@ -182,7 +183,8 @@ static void mn_itree_inv_end(struct mmu_notifier_mm *mm=
n_mm)
  */
 unsigned long mmu_interval_read_begin(struct mmu_interval_notifier *mni)
 {
-	struct mmu_notifier_mm *mmn_mm =3D mni->mm->mmu_notifier_mm;
+	struct mmu_notifier_subscriptions *subscriptions =3D
+		mni->mm->notifier_subscriptions;
 	unsigned long seq;
 	bool is_invalidating;
=20
@@ -190,17 +192,18 @@ unsigned long mmu_interval_read_begin(struct mmu_inte=
rval_notifier *mni)
...
+	 * colliding.
 	 ...
 	 *                                         spin_lock
 	 *                                          seq =3D READ_ONCE(mni->invali=
date_seq);
-	 *                                          seq =3D=3D mmn_mm->invalidate=
_seq
+	 *                                          seq =3D=3D subs->invalidate_s=
eq
 	 *                                         spin_unlock
 	 *    spin_lock
-	 *     seq =3D ++mmn_mm->invalidate_seq
+	 *     seq =3D ++subscriptions->invalidate_seq
 	 ...
 	 *     op->invalidate_range():
 	 *       user_lock
@@ -212,7 +215,7 @@ unsigned long mmu_interval_read_begin(struct mmu_interv=
al_notifier *mni)
 	 *
 	 *   mn_itree_inv_end():
 	 *    spin_lock
...
+	 *     seq =3D ++subscriptions->invalidate_seq
 	 ...
 	 *
 	 *                                        user_lock
@@ -224,24 +227,24 @@ unsigned long mmu_interval_read_begin(struct mmu_inte=
rval_notifier *mni)
...
+	spin_lock(&subscriptions->lock);
 	...
 	seq =3D READ_ONCE(mni->invalidate_seq);
-	is_invalidating =3D seq =3D=3D mmn_mm->invalidate_seq;
-	spin_unlock(&mmn_mm->lock);
+	is_invalidating =3D seq =3D=3D subscriptions->invalidate_seq;
+	spin_unlock(&subscriptions->lock);
=20
 	/*
...
+			   READ_ONCE(subscriptions->invalidate_seq) !=3D seq);
=20
 	/*
 	 * Notice that mmu_interval_read_retry() can already be true at this
@@ -253,7 +256,7 @@ unsigned long mmu_interval_read_begin(struct mmu_interv=
al_notifier *mni)
 }
...
@@ -267,13 +270,13 @@ static void mn_itree_release(struct mmu_notifier_mm *=
mmn_mm,
 	unsigned long cur_seq;
 	bool ret;
=20
-	for (mni =3D mn_itree_inv_start_range(mmn_mm, &range, &cur_seq); mni;
-	     mni =3D mn_itree_inv_next(mni, &range)) {
+	for (mni =3D mn_itree_inv_start_range(subscriptions, &range, &cur_seq);
+	     mni; mni =3D mn_itree_inv_next(mni, &range)) {
 		ret =3D mni->ops->invalidate(mni, &range, cur_seq);
 		WARN_ON(!ret);
 	}
=20
...
=20
 /*
@@ -283,12 +286,12 @@ static void mn_itree_release(struct mmu_notifier_mm *=
mmn_mm,
...
+ * against the other mmu notifiers with SRCU. struct mmu_notifier_subscrip=
tions
  * can't go away from under us as exit_mmap holds an mm_count pin
  * itself.
  */
-static void mn_hlist_release(struct mmu_notifier_mm *mmn_mm,
+static void mn_hlist_release(struct mmu_notifier_subscriptions *subscripti=
ons,
 			     struct mm_struct *mm)
 {
 	struct mmu_notifier *mn;
@@ -299,7 +302,7 @@ static void mn_hlist_release(struct mmu_notifier_mm *mm=
n_mm,
 	 * ->release returns.
 	 */
...
@@ -309,9 +312,9 @@ static void mn_hlist_release(struct mmu_notifier_mm *mm=
n_mm,
 		if (mn->ops->release)
...
-		mn =3D hlist_entry(mmn_mm->list.first, struct mmu_notifier,
+	spin_lock(&subscriptions->lock);
+	while (unlikely(!hlist_empty(&subscriptions->list))) {
+		mn =3D hlist_entry(subscriptions->list.first, struct mmu_notifier,
 				 hlist);
 		/*
 		 * We arrived before mmu_notifier_unregister so
@@ -321,7 +324,7 @@ static void mn_hlist_release(struct mmu_notifier_mm *mm=
n_mm,
 		 */
...
=20
 	/*
@@ -330,21 +333,22 @@ static void mn_hlist_release(struct mmu_notifier_mm *=
mmn_mm,
...
=20
 /*
@@ -360,7 +364,7 @@ int __mmu_notifier_clear_flush_young(struct mm_struct *=
mm,
 	int young =3D 0, id;
=20
 	id =3D srcu_read_lock(&srcu);
...
+	hlist_for_each_entry_rcu(mn, &mm->notifier_subscriptions->list, hlist) {
 		if (mn->ops->clear_flush_young)
 			young |=3D mn->ops->clear_flush_young(mn, mm, start, end);
 	}
...
+	hlist_for_each_entry_rcu(mn, &mm->notifier_subscriptions->list, hlist) {
 		if (mn->ops->clear_young)
 			young |=3D mn->ops->clear_young(mn, mm, start, end);
 	}
...
+	hlist_for_each_entry_rcu(mn, &mm->notifier_subscriptions->list, hlist) {
 		if (mn->ops->test_young) {
 			young =3D mn->ops->test_young(mn, mm, address);
 			if (young)
@@ -412,21 +416,22 @@ void __mmu_notifier_change_pte(struct mm_struct *mm, =
...
+static int mn_itree_invalidate(struct mmu_notifier_subscriptions *subscrip=
tions,
 			       const struct mmu_notifier_range *range)
 {
 	struct mmu_interval_notifier *mni;
...
=20
 		ret =3D mni->ops->invalidate(mni, range, cur_seq);
@@ -443,19 +448,20 @@ static int mn_itree_invalidate(struct mmu_notifier_mm=
 *mmn_mm,
...
+	hlist_for_each_entry_rcu(mn, &subscriptions->list, hlist) {
 		if (mn->ops->invalidate_range_start) {
 			int _ret;
=20
@@ -481,28 +487,29 @@ static int mn_hlist_invalidate_range_start(struct mmu=
_notifier_mm *mmn_mm,
=20
...
+	if (subscriptions->has_itree) {
+		ret =3D mn_itree_invalidate(subscriptions, range);
 		if (ret)
 			return ret;
 	}
-	if (!hlist_empty(&mmn_mm->list))
...
@@ -534,14 +541,15 @@ static void mn_hlist_invalidate_end(struct mmu_notifi=
er_mm *mmn_mm,
...
=20
@@ -552,7 +560,7 @@ void __mmu_notifier_invalidate_range(struct mm_struct *=
mm,
 	int id;
=20
 	id =3D srcu_read_lock(&srcu);
...
@@ -566,7 +574,7 @@ void __mmu_notifier_invalidate_range(struct mm_struct *=
mm,
  */
 int __mmu_notifier_register(struct mmu_notifier *mn, struct mm_struct *mm)
 {
-	struct mmu_notifier_mm *mmu_notifier_mm =3D NULL;
+	struct mmu_notifier_subscriptions *subscriptions =3D NULL;
 	int ret;
=20
 	lockdep_assert_held_write(&mm->mmap_sem);
@@ -579,23 +587,23 @@ int __mmu_notifier_register(struct mmu_notifier *mn, =
struct mm_struct *mm)
...
+		subscriptions->invalidate_seq =3D 2;
+		subscriptions->itree =3D RB_ROOT_CACHED;
+		init_waitqueue_head(&subscriptions->wq);
+		INIT_HLIST_HEAD(&subscriptions->deferred_list);
 	}
=20
 	ret =3D mm_take_all_locks(mm);
@@ -610,15 +618,16 @@ int __mmu_notifier_register(struct mmu_notifier *mn, =
struct mm_struct *mm)
...
@@ -626,18 +635,19 @@ int __mmu_notifier_register(struct mmu_notifier *mn, =
struct mm_struct *mm)
 		mn->mm =3D mm;
 		mn->users =3D 1;
=20
...
-		mm->mmu_notifier_mm->has_itree =3D true;
+		mm->notifier_subscriptions->has_itree =3D true;
=20
...
@@ -677,8 +687,9 @@ find_get_mmu_notifier(struct mm_struct *mm, const struc=
t mmu_notifier_ops *ops)
 {
 	struct mmu_notifier *mn;
=20
-	spin_lock(&mm->mmu_notifier_mm->lock);
...
+				 hlist) {
 		if (mn->ops !=3D ops)
 			continue;
=20
@@ -686,10 +697,10 @@ find_get_mmu_notifier(struct mm_struct *mm, const str=
uct mmu_notifier_ops *ops)
 			mn->users++;
 		else
 			mn =3D ERR_PTR(-EOVERFLOW);
-		spin_unlock(&mm->mmu_notifier_mm->lock);
+		spin_unlock(&mm->notifier_subscriptions->lock);
 		return mn;
 	}
...
=20
@@ -718,7 +729,7 @@ struct mmu_notifier *mmu_notifier_get_locked(const stru=
ct mmu_notifier_ops *ops,
=20
 	lockdep_assert_held_write(&mm->mmap_sem);
=20
-	if (mm->mmu_notifier_mm) {
+	if (mm->notifier_subscriptions) {
 		mn =3D find_get_mmu_notifier(mm, ops);
 		if (mn)
 			return mn;
@@ -739,11 +750,11 @@ struct mmu_notifier *mmu_notifier_get_locked(const st=
ruct mmu_notifier_ops *ops,
...
-	mm->mmu_notifier_mm =3D LIST_POISON1; /* debug */
+	BUG_ON(!hlist_empty(&mm->notifier_subscriptions->list));
+	kfree(mm->notifier_subscriptions);
+	mm->notifier_subscriptions =3D LIST_POISON1; /* debug */
 }
=20
 /*
@@ -776,13 +787,13 @@ void mmu_notifier_unregister(struct mmu_notifier *mn,=
 struct mm_struct *mm)
 			mn->ops->release(mn, mm);
 		srcu_read_unlock(&srcu, id);
=20
...
+		mni->invalidate_seq =3D subscriptions->invalidate_seq;
 	} else {
...
+		WARN_ON(mn_itree_is_invalidating(subscriptions));
 		 ...
 		 * soon.
 		 */
-		mni->invalidate_seq =3D mmn_mm->invalidate_seq - 1;
-		interval_tree_insert(&mni->interval_tree, &mmn_mm->itree);
+		mni->invalidate_seq =3D subscriptions->invalidate_seq - 1;
+		interval_tree_insert(&mni->interval_tree,
+				     &subscriptions->itree);
 	}
-	spin_unlock(&mmn_mm->lock);
...
=20
@@ -930,20 +942,20 @@ int mmu_interval_notifier_insert(struct mmu_interval_=
notifier *mni,
...
+		subscriptions =3D mm->notifier_subscriptions;
 	}
...
@@ -997,14 +1010,15 @@ void mmu_interval_notifier_remove(struct mmu_interva=
l_notifier *mni)
 			hlist_del(&mni->deferred_item);
 		} else {
 			hlist_add_head(&mni->deferred_item,
...
@@ -1013,8 +1027,8 @@ void mmu_interval_notifier_remove(struct mmu_interval=
_notifier *mni)
 	lock_map_acquire(&__mmu_notifier_invalidate_range_start_map);
 	lock_map_release(&__mmu_notifier_invalidate_range_start_map);
 	if (seq)
