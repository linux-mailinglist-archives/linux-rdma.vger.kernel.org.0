...
Received: from vger.kernel.org (vger.kernel.org [23.128.96.18])
	by mail.lfdr.de (Postfix) with ESMTP id BB2C8408AAA
	for <lists+linux-rdma@lfdr.de>; Mon, 13 Sep 2021 14:04:26 +0200 (CEST)
Received: (majordomo@vger.kernel.org) by vger.kernel.org via listexpand
        id S239705AbhIMMFk (ORCPT <rfc822;lists+linux-rdma@lfdr.de>);
        Mon, 13 Sep 2021 08:05:40 -0400
Received: from smtp-fw-80007.amazon.com ([99.78.197.218]:52002 "EHLO
        smtp-fw-80007.amazon.com" rhost-flags-OK-OK-OK-OK) by vger.kernel.org
        with ESMTP id S235453AbhIMMFk (ORCPT
        <rfc822;linux-rdma@vger.kernel.org>); Mon, 13 Sep 2021 08:05:40 -0400
DKIM-Signature: v=1; a=rsa-sha256; c=relaxed/relaxed;
  d=amazon.com; i=@amazon.com; q=dns/txt; s=amazon201209;
  t=1631534666; x=1663070666;
  h=from:to:cc:subject:date:message-id:mime-version:
   content-transfer-encoding;
  bh=DteHDnEmU4DOQ0HzSdX2BCRTLS3ulXgH7WGbLrinkQs=;
  b=FRS7c5Icby78/7VyiPiMe7G4HVS8rG5O3FzY5JQs8HvjJFWyBtXz+Wht
   vrmMZNoU2Qes3N4HWqLFVomH3AoVN5HeRetSSDASzTVv/j7Iw6Lnn1zrx
   XwC/m5Cb83ujiOKem8nsfxOCvWuUyJt9qT/UUwb08b3cLE41cRn0Q3EYH
   s=;
X-IronPort-AV: E=Sophos;i="5.85,288,1624320000"; 
   d="scan'208";a="26367684"
Received: from pdx4-co-svc-p1-lb2-vlan2.amazon.com (HELO email-inbound-relay-1d-db82c96c.us-east-1.amazon.com) ([10.25.36.210])
  by smtp-border-fw-80007.pdx80.corp.amazon.com with ESMTP; 13 Sep 2021 12:04:18 +0000
Received: from EX13D19EUA001.ant.amazon.com (iad12-ws-svc-p26-lb9-vlan3.iad.amazon.com [10.40.163.38])
        by email-inbound-relay-1d-db82c96c.us-east-1.amazon.com (Postfix) with ESMTPS id CC72CA2895;
        Mon, 13 Sep 2021 12:04:15 +0000 (UTC)
Received: from EX13MTAUEE002.ant.amazon.com (10.43.62.24) by
 EX13D19EUA001.ant.amazon.com (10.43.165.74) with Microsoft SMTP Server (TLS)
 id 15.0.1497.23; Mon, 13 Sep 2021 12:04:14 +0000
Received: from 8c85908914bf.ant.amazon.com.com (10.1.212.33) by
 mail-relay.amazon.com (10.43.62.224) with Microsoft SMTP Server id
 15.0.1497.23 via Frontend Transport; Mon, 13 Sep 2021 12:04:11 +0000
...
Subject: [PATCH for-next v2] RDMA/efa: CQ notifications
Date:   Mon, 13 Sep 2021 15:04:04 +0300
Message-ID: <20210913120406.61745-1-galpress@amazon.com>
...
---
 ...
 drivers/infiniband/hw/efa/efa_main.c          | 184 +++++++++++++++---
 drivers/infiniband/hw/efa/efa_regs_defs.h     |   7 +-
 drivers/infiniband/hw/efa/efa_verbs.c         |  63 +++++-
 include/uapi/rdma/efa-abi.h                   |  18 +-
 11 files changed, 630 insertions(+), 55 deletions(-)

diff --git a/drivers/infiniband/hw/efa/efa.h b/drivers/infiniband/hw/efa/efa.h
index 87b1dadeb7fe..88e13b0c319a 100644
...
index a27ff0c12e0a..dc9b05c46920 100644
...
@@ -77,26 +117,43 @@ static irqreturn_t efa_intr_msix_mgmnt(int irq, void *data)
 	return IRQ_HANDLED;
 }
...
+
 ...
 {
 	u32 cpu;
@@ -105,8 +162,9 @@ static void efa_setup_mgmnt_irq(struct efa_dev *dev)
...
@@ -115,20 +173,11 @@ static void efa_setup_mgmnt_irq(struct efa_dev *dev)
 		 dev->admin_irq.name);
 }
...
@@ -234,6 +283,76 @@ static void efa_set_host_info(struct efa_dev *dev)
...
@@ -300,23 +419,29 @@ static int efa_ib_device_add(struct efa_dev *dev)
 	if (err)
...
+	efa_destroy_eqs(dev);
 err_release_doorbell_bar:
 	...
 	return err;
@@ -324,9 +449,10 @@ static int efa_ib_device_add(struct efa_dev *dev)
...
@@ -339,8 +465,12 @@ static int efa_enable_msix(struct efa_dev *dev)
 {
...
@@ -420,6 +550,7 @@ static struct efa_dev *efa_probe_device(struct pci_dev *pdev)
 	edev->efa_dev = dev;
...
@@ -475,7 +606,7 @@ static struct efa_dev *efa_probe_device(struct pci_dev *pdev)
 	return dev;
...
@@ -498,11 +629,12 @@ static void efa_remove_device(struct pci_dev *pdev)
...
index e5f9d90aad5e..eaa4f5fd5379 100644
...
@@ -1011,6 +1029,21 @@ static int cq_mmap_entries_setup(struct efa_dev *dev, struct efa_cq *cq,
 	if (!cq->mmap_entry)
 		return -ENOMEM;
...
@@ -1019,8 +1052,8 @@ int efa_create_cq(struct ib_cq *ibcq, const struct ib_cq_init_attr *attr,
 {
...
@@ -1065,7 +1098,7 @@ int efa_create_cq(struct ib_cq *ibcq, const struct ib_cq_init_attr *attr,
 		goto err_out;
 	}
...
+	if (cmd.comp_mask || !is_reserved_cleared(cmd.reserved_58)) {
 		...
 		err = -EINVAL;
@@ -1101,29 +1134,43 @@ int efa_create_cq(struct ib_cq *ibcq, const struct ib_cq_init_attr *attr,
...
@@ -1132,8 +1179,10 @@ int efa_create_cq(struct ib_cq *ibcq, const struct ib_cq_init_attr *attr,
