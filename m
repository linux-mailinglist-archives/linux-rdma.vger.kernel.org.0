...
Received: from vger.kernel.org (vger.kernel.org [23.128.96.18])
	by mail.lfdr.de (Postfix) with ESMTP id 6B957301177
	for <lists+linux-rdma@lfdr.de>; Sat, 23 Jan 2021 01:15:36 +0100 (CET)
Received: (majordomo@vger.kernel.org) by vger.kernel.org via listexpand
        id S1726082AbhAWAOq (ORCPT <rfc822;lists+linux-rdma@lfdr.de>);
        Fri, 22 Jan 2021 19:14:46 -0500
Received: from mga05.intel.com ([192.55.52.43]:55260 "EHLO mga05.intel.com"
        rhost-flags-OK-OK-OK-OK) by vger.kernel.org with ESMTP
        id S1726412AbhAVXwX (ORCPT <rfc822;linux-rdma@vger.kernel.org>);
        Fri, 22 Jan 2021 18:52:23 -0500
IronPort-SDR: 3PkRDluBsfm+l79QKFe0qrPHcSsF7+AbYCBpmxmbwTPLvR8ta5OMS3FXQiodwin1tB/G+QOmf9
 GwsmMloGXgEw==
X-IronPort-AV: E=McAfee;i="6000,8403,9872"; a="264346882"
X-IronPort-AV: E=Sophos;i="5.79,368,1602572400"; 
   d="scan'208";a="264346882"
Received: from fmsmga005.fm.intel.com ([10.253.24.32])
  by fmsmga105.fm.intel.com with ESMTP/TLS/ECDHE-RSA-AES256-GCM-SHA384; 22 Jan 2021 15:50:17 -0800
IronPort-SDR: dKZyKfmJm9jFfdwjINMn5GtKY6jbPQ8qf9NhxzwzxOflFddi2rsKgmORvQ5IxoAm3qA5WVSK/o
 wQw3expAnTFA==
X-IronPort-AV: E=Sophos;i="5.79,368,1602572400"; 
   d="scan'208";a="574869475"
Received: from ssaleem-mobl.amr.corp.intel.com ([10.251.4.95])
  by fmsmga005-auth.fm.intel.com with ESMTP/TLS/ECDHE-RSA-AES256-GCM-SHA384; 22 Jan 2021 15:50:14 -0800
...
Subject: [PATCH 09/22] RDMA/irdma: Implement HW Admin Queue OPs
Date:   Fri, 22 Jan 2021 17:48:14 -0600
Message-Id: <20210122234827.1353-10-shiraz.saleem@intel.com>
...
---
 ...
 4 files changed, 10242 insertions(+)
...
+	irdma_dbg(qp->dev, "DCB: DCB: Remove qp[%d] UP[%d] qset[%d]\n",
...
+	mutex_unlock(&vsi->qos[qp->user_pri].qos_mutex);
+	irdma_dbg(qp->dev, "DCB: DCB: Add qp[%d] UP[%d] qset[%d]\n",
+		  qp->qp_uk.qp_id, qp->user_pri, qp->qs_handle);
+}
+
+/**
...
+	irdma_dbg(qp->dev, "WQE: hw_sq_size[%04d] sq_ring.size[%04d]\n",
...
+
+	irdma_dbg(qp->dev, "MR: wr_id[%llxh] wqe_idx[%04d] location[%p]\n",
+		  info->wr_id, wqe_idx,
...
+	stats_buff_mem->va = dma_alloc_coherent(ihw_to_dev(vsi->pestat->hw),
+						stats_buff_mem->size,
+						&stats_buff_mem->pa,
+						GFP_KERNEL);
...
+
+stats_error:
+	dma_free_coherent(ihw_to_dev(vsi->pestat->hw), stats_buff_mem->size,
...
+	dma_free_coherent(ihw_to_dev(vsi->pestat->hw),
...
+
+	irdma_sc_cqp_post_sq(cqp);
+	irdma_dbg(cqp->dev, "STATS: CQP SQ head 0x%x tail 0x%x size 0x%x\n",
...
+	cqp->sdbuf.va = dma_alloc_coherent(ihw_to_dev(cqp->dev->hw),
...
+
+err:
+	dma_free_coherent(ihw_to_dev(cqp->dev->hw), cqp->sdbuf.size,
...
+
+	irdma_dbg(cqp->dev, "WQE: CQP SQ head 0x%x tail 0x%x size 0x%x\n",
+		  cqp->sq_ring.head, cqp->sq_ring.tail, cqp->sq_ring.size);
+}
+
+/**
...
+		error = readl(cqp->dev->hw_regs[IRDMA_CQPERRCODES]);
+		irdma_dbg(cqp->dev, "CQP: CQPERRCODES error_code[x%08X]\n",
+			  error);
+	}
+
...
+		if (cq == next_cq)
+			set_64bit_val(ceqe, 0, temp & IRDMA_CEQE_VALID_M);
+
...
+
+	irdma_dbg(dev, "HMC: hmc_fn_id %u, dev->hmc_fn_id %u\n", hmc_fn_id,
+		  dev->hmc_fn_id);
...
+	}
+	if (sd > 0xFFFFFFFF) {
+		irdma_dbg(dev, "HMC: sd overflow[%lld]\n", sd);
+		sd = 0xFFFFFFFF - 1;
+	}
+
+	return (u32)sd;
...
+	feat_buf.va = dma_alloc_coherent(ihw_to_dev(dev->hw), feat_buf.size,
...
+		dma_free_coherent(ihw_to_dev(dev->hw), feat_buf.size,
+				  feat_buf.va, feat_buf.pa);
+		feat_buf.va = NULL;
+		feat_buf.size = ALIGN(8 * feat_cnt,
+				      IRDMA_FEATURE_BUF_ALIGNMENT);
+		feat_buf.va = dma_alloc_coherent(ihw_to_dev(dev->hw),
...
+	}
+exit:
+	dma_free_coherent(ihw_to_dev(dev->hw), feat_buf.size, feat_buf.va,
+			  feat_buf.pa);
+	feat_buf.va = NULL;
+	return ret_code;
+}
...
+	irdma_dbg(dev, "HMC: sd count %d where max sd is %d\n",
...
+	if (ret_code) {
+		irdma_dbg(dev, "HMC: cfg_iw_fpm returned error_code[x%08X]\n",
...
+		reg_val = interval & IRDMA_GLINT_RATE_INTERVAL_M;
+		reg_val |= 1 << IRDMA_GLINT_RATE_INTRL_ENA_S;
+	}
