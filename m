...
Received: from vger.kernel.org (vger.kernel.org [23.128.96.18])
	by mail.lfdr.de (Postfix) with ESMTP id DFA082C8B41
	for <lists+linux-rdma@lfdr.de>; Mon, 30 Nov 2020 18:38:07 +0100 (CET)
Received: (majordomo@vger.kernel.org) by vger.kernel.org via listexpand
        id S2387578AbgK3Rf5 (ORCPT <rfc822;lists+linux-rdma@lfdr.de>);
        Mon, 30 Nov 2020 12:35:57 -0500
Received: from mga05.intel.com ([192.55.52.43]:16934 "EHLO mga05.intel.com"
        rhost-flags-OK-OK-OK-OK) by vger.kernel.org with ESMTP
        id S2387505AbgK3Rf4 (ORCPT <rfc822;linux-rdma@vger.kernel.org>);
        Mon, 30 Nov 2020 12:35:56 -0500
IronPort-SDR: xyUD/Lk/AtmzIqCysGgLeVYjViF0tA+rziHbRNvcO2bBi70obOwrxKCcUrYjSS9pK2pkiBKVZA
 pgWVeDNg/1DQ==
X-IronPort-AV: E=McAfee;i="6000,8403,9821"; a="257383473"
X-IronPort-AV: E=Sophos;i="5.78,382,1599548400"; 
   d="scan'208";a="257383473"
X-Amp-Result: SKIPPED(no attachment in message)
X-Amp-File-Uploaded: False
Received: from orsmga006.jf.intel.com ([10.7.209.51])
  by fmsmga105.fm.intel.com with ESMTP/TLS/ECDHE-RSA-AES256-GCM-SHA384; 30 Nov 2020 09:35:12 -0800
IronPort-SDR: 7BaNVBlTf7vi+NwOqDXQJZ5SPFj6+Ktf6ToqZtbD2JWnlfKuLsSQwXsuXGb74n8CQaKwv+ErBo
 G4fhgItY1s8Q==
X-ExtLoop1: 1
X-IronPort-AV: E=Sophos;i="5.78,382,1599548400"; 
   d="scan'208";a="334738414"
...
X-MS-Exchange-CrossTenant-originalarrivaltime: 30 Nov 2020 17:35:06.3046
 (UTC)
...
> Cc: linux-rdma@vger.kernel.org; dri-devel@lists.freedesktop.org; Leon Rom=
anovsky <leon@kernel.org>; Jason Gunthorpe <jgg@ziepe.ca>;
...
> >                          dm_mr =3D DMMR(pd, dm_mr_len, e.IBV_ACCESS_ZER=
O_BASED,
> >                                       dm=3Ddm, offset=3Ddm_mr_offset)
> >                          dm_mr.close()
> > +
> > +
...
> > +        DmaBuf(1)
>=20
> Hardcoding gpu unit 1 here (and in other places) is probably not quite wh=
at we want. Not sure what you want to do in the test framework
> here instead.

'1' here is the buffer size. Unit is the default value 0. We could probably=
 add a command line argument to the test to set the preferred gpu unit.

>=20
> > +    except PyverbsRDMAError as ex:
> > +        if ex.error_code =3D=3D errno.ENOENT:
> > +            raise unittest.SkipTest('Device /dev/dri/renderD* is not p=
resent')
> > +        if ex.error_code =3D=3D errno.EACCES:
...
> > +        DmaBufMR(pd, 1, 0)
> > +    except PyverbsRDMAError as ex:
> > +        if ex.error_code =3D=3D errno.EOPNOTSUPP:
...
> > +                flags =3D u.get_dmabuf_access_flags(ctx)
> > +                for f in flags:
> > +                    len =3D u.get_mr_length()
> > +                    for off in [0, len//2]:
> > +                        with DmaBufMR(pd, len, f, offset=3Doff) as mr:
> > +                            pass
> > +
...
> > +                check_dmabuf_mr_support(pd)
> > +                for i in range(5):
> > +                    flags =3D random.sample([e.IBV_ACCESS_REMOTE_WRITE=
,
...
> > +                        assert 'Failed to register a dma-buf MR' in er=
r.args[0]
> > +                    else:
...
> > +                check_dmabuf_mr_support(pd)
> > +                for i in range(10):
> > +                    mr_len =3D u.get_mr_length()
> > +                    flags =3D u.get_dmabuf_access_flags(ctx)
> > +                    for f in flags:
> > +                        for mr_off in [0, mr_len//2]:
> > +                            with DmaBufMR(pd, mr_len, f, offset=3Dmr_o=
ff) as mr:
> > +                                write_len =3D min(random.randint(1, MA=
X_IO_LEN),
> > +                                                mr_len)
...
X_IO_LEN),
> > +                                                mr_len)
> > +                                write_str =3D 'a' * write_len
> > +                                mr.write(write_str, write_len)
> > +                                read_len =3D random.randint(1, write_l=
en)
> > +                                offset =3D random.randint(0, write_len=
-read_len)
> > +                                read_str =3D mr.read(read_len, offset)=
.decode()
...
> > +        """
> > +        super(DmaBufRC, self).__init__(dev_name=3Ddev_name, ib_port=3D=
ib_port,
...
> > +        access =3D e.IBV_ACCESS_LOCAL_WRITE | e.IBV_ACCESS_REMOTE_WRIT=
E
> > +        mr =3D DmaBufMR(self.pd, self.msg_size, access)
> > +        self.mr =3D mr
> > +
> > +    def create_qp_attr(self):
> > +        qp_attr =3D QPAttr(port_num=3Dself.ib_port)
> > +        qp_access =3D e.IBV_ACCESS_LOCAL_WRITE | e.IBV_ACCESS_REMOTE_W=
RITE
> > +        qp_attr.qp_access_flags =3D qp_access
> > +        return qp_attr
> > +
> > +
...
> > +                         BaseResources.
> > +        :param resource_arg: Dict of args that specify the resource sp=
ecific
> > +                             attributes.
> > +        :return: The (client, server) resources.
> > +        """
> > +        client =3D resource(**self.dev_info, **resource_arg)
> > +        server =3D resource(**self.dev_info, **resource_arg)
...
> > +        """
> > +        client, server =3D self.create_players(DmaBufRC)
...
> > +        server.remote_addr =3D client.mr.offset
> > +        client.rkey =3D server.mr.rkey
> > +        client.remote_addr =3D server.mr.offset
> > +        u.rdma_traffic(client, server, self.iters, self.gid_index, sel=
f.ib_port,
> > +                       send_op=3De.IBV_WR_RDMA_WRITE)
...
> > +    """
> > +    attr =3D ctx.query_device()
> > +    vals =3D [e.IBV_ACCESS_LOCAL_WRITE, e.IBV_ACCESS_REMOTE_WRITE,
...
> > +        tmp =3D list(com(vals, i))
> > +        tmp =3D filter(filter_illegal_access_flags, tmp)
> > +        for t in tmp:  # Iterate legal combinations and bitwise OR the=
m
> > +            val =3D 0
> > +            for flag in t:
> > +                val +=3D flag.value
