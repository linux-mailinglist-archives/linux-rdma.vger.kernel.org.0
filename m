...
Received: from vger.kernel.org (vger.kernel.org [23.128.96.18])
	by mail.lfdr.de (Postfix) with ESMTP id E3E421DAB74
	for <lists+linux-rdma@lfdr.de>; Wed, 20 May 2020 09:06:37 +0200 (CEST)
Received: (majordomo@vger.kernel.org) by vger.kernel.org via listexpand
        id S1727004AbgETHEb (ORCPT <rfc822;lists+linux-rdma@lfdr.de>);
        Wed, 20 May 2020 03:04:31 -0400
Received: from mga12.intel.com ([192.55.52.136]:50891 "EHLO mga12.intel.com"
        rhost-flags-OK-OK-OK-OK) by vger.kernel.org with ESMTP
        id S1726990AbgETHEa (ORCPT <rfc822;linux-rdma@vger.kernel.org>);
        Wed, 20 May 2020 03:04:30 -0400
IronPort-SDR: wuJ/aIFrNS8HAGl10AkCpC97AHiAmPkl5OOSdglRi8JlB7oiwIKFpRizTpMih4nWtqIZuYLOVy
 pPcaETIzoaAQ==
X-Amp-Result: SKIPPED(no attachment in message)
X-Amp-File-Uploaded: False
Received: from orsmga003.jf.intel.com ([10.7.209.27])
  by fmsmga106.fm.intel.com with ESMTP/TLS/ECDHE-RSA-AES256-GCM-SHA384; 20 May 2020 00:04:18 -0700
IronPort-SDR: AY0LSkCtf2nu2i36ilYRJSsuop2hLuRj1cADWKu0+mgREtDgnomOR1OpPCaXwdTlTj9wGlp7jS
 teojtSrLVKOQ==
X-ExtLoop1: 1
X-IronPort-AV: E=Sophos;i="5.73,413,1583222400"; 
   d="scan'208";a="264581226"
Received: from jtkirshe-desk1.jf.intel.com ([134.134.177.86])
  by orsmga003.jf.intel.com with ESMTP; 20 May 2020 00:04:17 -0700
...
Subject: [RDMA RFC v6 05/16] RDMA/irdma: Add privileged UDA queue implementation
Date:   Wed, 20 May 2020 00:04:04 -0700
Message-Id: <20200520070415.3392210-6-jeffrey.t.kirsher@intel.com>
...
index 000000000000..8feb5f610176
--- /dev/null
...
+	dma_sync_single_for_device(ihw_to_dev(rsrc->dev->hw), buf->mem.pa,
...
+	if (!buf->mem.va)
+		goto free_virt;
+	buf->mem.pa = dma_map_single(ihw_to_dev(dev->hw), buf->mem.va,
+				     buf->mem.size, DMA_BIDIRECTIONAL);
+	if (dma_mapping_error(ihw_to_dev(dev->hw), buf->mem.pa)) {
+		kfree(buf->mem.va);
+		goto free_virt;
+	}
+
...
+				struct irdma_puda_buf *buf)
+{
+	dma_unmap_single(ihw_to_dev(dev->hw), buf->mem.pa, buf->mem.size,
+			 DMA_BIDIRECTIONAL);
+	kfree(buf->mem.va);
...
+	if (error) {
+		irdma_dbg(cq->dev, "PUDA: receive error\n");
...
+							cq->vsi->ieq;
+	} else {
+		irdma_dbg(dev, "PUDA: qp_type error\n");
...
+		dma_sync_single_for_cpu(ihw_to_dev(dev->hw), buf->mem.pa,
+					buf->mem.size, DMA_BIDIRECTIONAL);
...
+		irdma_dbg(dev, "PUDA: RQ completion\n");
...
+
+	} else {
+		irdma_dbg(dev, "PUDA: SQ completion\n");
+		buf = (struct irdma_puda_buf *)(uintptr_t)
...
+			irdma_dbg(rsrc->dev, "PUDA: adding to txpend\n");
+		return;
+	}
+	rsrc->tx_wqe_avail_cnt--;
...
+
+	/* Synch buffer for use by device */
+	dma_sync_single_for_cpu(ihw_to_dev(rsrc->dev->hw), buf->mem.pa,
+				buf->mem.size, DMA_BIDIRECTIONAL);
...
+		if (rsrc->type == IRDMA_PUDA_RSRC_TYPE_ILQ)
+			irdma_dbg(rsrc->dev, "PUDA: adding to puda_send\n");
+	} else {
+		rsrc->stats_pkt_sent++;
+	}
+done:
...
+				 IRDMA_HW_PAGE_SIZE);
+	rsrc->qpmem.va = dma_alloc_coherent(ihw_to_dev(rsrc->dev->hw),
...
+	if (ret) {
+		dma_free_coherent(ihw_to_dev(rsrc->dev->hw), rsrc->qpmem.size,
...
+				 IRDMA_CQ0_ALIGNMENT);
+	rsrc->cqmem.va = dma_alloc_coherent(ihw_to_dev(dev->hw),
...
+error:
+	if (ret) {
+		dma_free_coherent(ihw_to_dev(dev->hw), rsrc->cqmem.size,
...
+	if (ret)
+		irdma_dbg(dev, "PUDA: error ieq cq destroy\n");
+	if (!ret) {
+		ret = dev->cqp_ops->poll_for_cqp_op_done(dev->cqp,
+							 IRDMA_CQP_OP_DESTROY_CQ,
+							 &compl_info);
+		if (ret)
+			irdma_dbg(dev, "PUDA: error ieq qp destroy done\n");
+	}
+}
+
+/**
...
+		break;
+	default:
+		irdma_dbg(dev, "PUDA: error resource type = 0x%x\n", type);
+		return;
+	}
+
+	switch (rsrc->cmpl) {
...
+
+		dma_free_coherent(ihw_to_dev(dev->hw), rsrc->qpmem.size,
+				  rsrc->qpmem.va, rsrc->qpmem.pa);
+		rsrc->qpmem.va = NULL;
+		/* fallthrough */
...
+		break;
+	default:
+		irdma_dbg(rsrc->dev, "PUDA: error no resources\n");
+		break;
+	}
...
+	}
+	if (ret) {
+		irdma_dbg(dev, "PUDA: error qp_create type=%d, status=%d\n",
+			  rsrc->type, ret);
+		goto error;
+	}
...
+
+	if (ret) {
+		irdma_dbg(dev, "PUDA: error unable to add to cq_ctx\n");
+		goto error;
+	}
+
+	ret = irdma_puda_allocbufs(rsrc, info->tx_buf_cnt + info->rq_size);
+	if (ret) {
+		irdma_dbg(dev, "PUDA: error alloc_buf\n");
+		goto error;
+	}
+
...
+	struct irdma_puda_buf *buf = (struct irdma_puda_buf *)(uintptr_t)
+					    qp->qp_uk.rq_wrid_array[wqe_idx];
+
+	/* Synch buffer for use by device */
+	dma_sync_single_for_device(ihw_to_dev(qp->dev->hw), buf->mem.pa,
+				   buf->mem.size, DMA_BIDIRECTIONAL);
+	wqe = qp->qp_uk.rq_base[wqe_idx].elem;
+	get_64bit_val(wqe, 24, &offset24);
+	if (qp->dev->hw_attrs.uk_attrs.hw_rev >= IRDMA_GEN_2) {
...
+						(fpdu_len - 4), mpacrc);
+		if (status) {
+			irdma_dbg(ieq->dev, "IEQ: error bad crc\n");
+			goto error;
+		}
+	}
+
...
+		if (!fpdu_len) {
+			irdma_dbg(ieq->dev, "IEQ: error bad fpdu len\n");
+			list_add(&buf->list, rxlist);
...
+		if (ret) {
+			list_add(&buf->list, rxlist);
+			irdma_dbg(ieq->dev, "ERR: IRDMA_ERR_MPA_CRC\n");
+			return IRDMA_ERR_MPA_CRC;
+		}
+		full++;
...
+		if (!buf) {
+			irdma_dbg(ieq->dev, "IEQ: error no buf\n");
+			break;
+		}
...
+		irdma_dbg(ieq->dev, "IEQ: restarting new partial\n");
+		pfpdu->mode = false;
+	}
+
+	if (!pfpdu->mode) {
...
new file mode 100644
index 000000000000..92b48a645002
--- /dev/null
