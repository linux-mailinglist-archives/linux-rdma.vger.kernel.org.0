...
Received: from out1.vger.email (out1.vger.email [IPv6:2620:137:e000::1:20])
	by mail.lfdr.de (Postfix) with ESMTP id A864F691DCA
	for <lists+linux-rdma@lfdr.de>; Fri, 10 Feb 2023 12:12:00 +0100 (CET)
Received: (majordomo@vger.kernel.org) by vger.kernel.org via listexpand
        id S232386AbjBJLL5 (ORCPT <rfc822;lists+linux-rdma@lfdr.de>);
        Fri, 10 Feb 2023 06:11:57 -0500
Received: from lindbergh.monkeyblade.net ([23.128.96.19]:51846 "EHLO
        lindbergh.monkeyblade.net" rhost-flags-OK-OK-OK-OK) by vger.kernel.org
        with ESMTP id S232369AbjBJLLx (ORCPT
        <rfc822;linux-rdma@vger.kernel.org>); Fri, 10 Feb 2023 06:11:53 -0500
Received: from mx0b-0016f401.pphosted.com (mx0b-0016f401.pphosted.com [67.231.156.173])
        by lindbergh.monkeyblade.net (Postfix) with ESMTPS id CFE9A70CC0;
        Fri, 10 Feb 2023 03:11:40 -0800 (PST)
Received: from pps.filterd (m0045851.ppops.net [127.0.0.1])
        by mx0b-0016f401.pphosted.com (8.17.1.19/8.17.1.19) with ESMTP id 31A83OkE004265;
        Fri, 10 Feb 2023 03:11:29 -0800
DKIM-Signature: v=1; a=rsa-sha256; c=relaxed/relaxed; d=marvell.com; h=from : to : cc :
 subject : date : message-id : in-reply-to : references : mime-version :
 content-type; s=pfpt0220; bh=a/4q1moFZfSBj4ae+i+rn+VmX/K6K8uc5U/PVkp+/eA=;
 b= tg== 
Received: from dc5-exch02.marvell.com ([199.233.59.182])
        by mx0b-0016f401.pphosted.com (PPS) with ESMTPS id 3nn71cm05f-1
        (version=TLSv1.2 cipher=ECDHE-RSA-AES256-SHA384 bits=256 verify=NOT);
        Fri, 10 Feb 2023 03:11:28 -0800
Received: from DC5-EXCH01.marvell.com (10.69.176.38) by DC5-EXCH02.marvell.com
 (10.69.176.39) with Microsoft SMTP Server (TLS) id 15.0.1497.42; Fri, 10 Feb
 2023 03:11:25 -0800
Received: from maili.marvell.com (10.69.176.80) by DC5-EXCH01.marvell.com
 (10.69.176.38) with Microsoft SMTP Server id 15.0.1497.42 via Frontend
 Transport; Fri, 10 Feb 2023 03:11:25 -0800
Received: from hyd1soter3.marvell.com (unknown [10.29.37.12])
        by maili.marvell.com (Postfix) with ESMTP id 7ABB93F7067;
...
Subject: [net-next Patch V4 4/4] octeontx2-pf: Add support for HTB offload
Date:   Fri, 10 Feb 2023 16:40:51 +0530
Message-ID: <20230210111051.13654-5-hkelam@marvell.com>
X-Mailer: git-send-email 2.17.1
In-Reply-To: <20230210111051.13654-1-hkelam@marvell.com>
References: <20230210111051.13654-1-hkelam@marvell.com>
MIME-Version: 1.0
Content-Type: text/plain
X-Proofpoint-ORIG-GUID: bYcgAQ3PFkO-CeEjz5_NvR-p2QKtvZ3I
X-Proofpoint-GUID: bYcgAQ3PFkO-CeEjz5_NvR-p2QKtvZ3I
...
@@ -747,29 +752,49 @@ int otx2_txsch_alloc(struct otx2_nic *pfvf)
 	return 0;
 }
...
index fc44e7de5158..240199225fe2 100644
...
index 9ed24bff6b2a..2f40f5751cd0 100644
...
@@ -1835,6 +1840,9 @@ int otx2_open(struct net_device *netdev)
...
+
+	dev_reset_queue(pfvf->netdev,
+			netdev_get_tx_queue(pfvf->netdev, pfvf->hw.tx_queues + moved_qid),
+			NULL);
+
...
index 0be049b191cd..6b4534a6f4fc 100644
...
index af4aa66aaa4e..65bf243cfc13 100644
--- a/include/net/sch_generic.h
+++ b/include/net/sch_generic.h
@@ -647,6 +647,8 @@ void dev_shutdown(struct net_device *dev);
 void dev_activate(struct net_device *dev);
 void dev_deactivate(struct net_device *dev);
 void dev_deactivate_many(struct list_head *head);
+void dev_reset_queue(struct net_device *dev, struct netdev_queue *dev_queue,
+		     void *_unused);
 struct Qdisc *dev_graft_qdisc(struct netdev_queue *dev_queue,
 			      struct Qdisc *qdisc);
 void qdisc_reset(struct Qdisc *qdisc);
diff --git a/net/sched/sch_generic.c b/net/sched/sch_generic.c
index a9aadc4e6858..4b7ab1b40640 100644
--- a/net/sched/sch_generic.c
+++ b/net/sched/sch_generic.c
@@ -1265,9 +1265,8 @@ static void dev_deactivate_queue(struct net_device *dev,
 	}
 }
