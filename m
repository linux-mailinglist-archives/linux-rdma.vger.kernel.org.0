...
Received: from vger.kernel.org (vger.kernel.org [23.128.96.18])
	by mail.lfdr.de (Postfix) with ESMTP id 8AAC430116C
	for <lists+linux-rdma@lfdr.de>; Sat, 23 Jan 2021 01:12:47 +0100 (CET)
Received: (majordomo@vger.kernel.org) by vger.kernel.org via listexpand
        id S1726535AbhAWAMT (ORCPT <rfc822;lists+linux-rdma@lfdr.de>);
        Fri, 22 Jan 2021 19:12:19 -0500
Received: from mga05.intel.com ([192.55.52.43]:55260 "EHLO mga05.intel.com"
        rhost-flags-OK-OK-OK-OK) by vger.kernel.org with ESMTP
        id S1726532AbhAVXxO (ORCPT <rfc822;linux-rdma@vger.kernel.org>);
        Fri, 22 Jan 2021 18:53:14 -0500
IronPort-SDR: apkhwhDK9SHxlnrDkxlLo0b5zgtuek/v/ErLbnoyWbzovatib38Xg76tWNIu/lGPCeTMzQR7+b
 5d5otSb4DGDw==
X-IronPort-AV: E=McAfee;i="6000,8403,9872"; a="264346896"
X-IronPort-AV: E=Sophos;i="5.79,368,1602572400"; 
   d="scan'208";a="264346896"
Received: from fmsmga005.fm.intel.com ([10.253.24.32])
  by fmsmga105.fm.intel.com with ESMTP/TLS/ECDHE-RSA-AES256-GCM-SHA384; 22 Jan 2021 15:50:23 -0800
IronPort-SDR: Yv5FquYs2rZC04BuAaUh2vMcoPvRhxTUbllY6Ocx98jHr2+3MQ3UpLN/hmS8YL5aieJjvjdKpo
 y4xPgbi88KTw==
X-IronPort-AV: E=Sophos;i="5.79,368,1602572400"; 
   d="scan'208";a="574869524"
Received: from ssaleem-mobl.amr.corp.intel.com ([10.251.4.95])
  by fmsmga005-auth.fm.intel.com with ESMTP/TLS/ECDHE-RSA-AES256-GCM-SHA384; 22 Jan 2021 15:50:21 -0800
...
Subject: [PATCH 13/22] RDMA/irdma: Add connection manager
Date:   Fri, 22 Jan 2021 17:48:18 -0600
Message-Id: <20210122234827.1353-14-shiraz.saleem@intel.com>
...
+
+	irdma_dbg(iwdev_to_idev(cm_node->iwdev),
...
+		irdma_dbg(iwdev_to_idev(cm_node->iwdev), "CM: AH invalid\n");
+		return NULL;
+	}
+
+	sqbuf = irdma_puda_get_bufpool(vsi->ilq);
+	if (!sqbuf) {
+		irdma_dbg(iwdev_to_idev(cm_node->iwdev), "CM: SQ buf NULL\n");
+		return NULL;
+	}
+
...
+	cm_node->state = IRDMA_CM_STATE_CLOSED;
+	irdma_dbg(iwdev_to_idev(cm_node->iwdev), "CM: cm_node=%p state =%d\n",
...
+		irdma_dbg(iwdev_to_idev(cm_node->iwdev), "CM: rev %d\n",
+			  mpa_frame->rev);
+		return -EINVAL;
+	}
+
...
+		if (!idev) {
+			irdma_dbg(iwdev_to_idev(iwdev), "CM: idev == NULL\n");
+			break;
+		}
...
+		irdma_dbg(iwdev_to_idev(listener->iwdev),
...
+	if (IS_ERR(rt)) {
+		irdma_dbg(iwdev_to_idev(iwdev), "CM: ip_route_output fail\n");
+		return -EINVAL;
+	}
+
...
+		goto exit;
+
+	irdma_dbg(iwdev_to_idev(iwdev), "CM: dst_neigh_lookup MAC=%pM\n",
+		  neigh->ha);
+
...
+		if (cm_node->loc_port == loc_port && cm_node->rem_port == rem_port &&
...
+					   VLAN_PRIO_SHIFT;
+			cm_info.vlan_id = vtag & VLAN_VID_MASK;
+			irdma_dbg(iwdev_to_idev(cm_core->iwdev),
+				  "CM: vlan_id=%d\n", cm_info.vlan_id);
+		} else {
+			cm_info.vlan_id = 0xFFFF;
+		}
...
+				  cm_info.loc_port, cm_info.loc_addr);
+
+	if (!cm_node) {
...
+	irdma_dbg(iwdev_to_idev(iwdev), "CM: Call close API\n");
...
+		dma_free_coherent(ihw_to_dev(iwdev->rf->sc_dev.hw),
...
+				       NULL);
+	}
+	irdma_dbg(iwdev_to_idev(iwdev), "CM: Accept vlan_id=%d\n",
+		  cm_node->vlan_id);
+
...
+	iwqp->ietf_mem.va = dma_alloc_coherent(ihw_to_dev(dev->hw),
+					       iwqp->ietf_mem.size,
...
+	irdma_qp_add_qos(&iwqp->sc_qp);
+	irdma_dbg(iwdev_to_idev(iwdev), "DCB: TOS:[%d] UP:[%d]\n", cm_id->tos,
+		  cm_info.user_pri);
+
...
+
+#define cmnode_to_idev(cm_node)	(&((cm_node)->iwdev->rf->sc_dev))
+
+enum ietf_mpa_flags {
