...
Received: from vger.kernel.org (vger.kernel.org [23.128.96.18])
	by mail.lfdr.de (Postfix) with ESMTP id 022A41DAB6F
	for <lists+linux-rdma@lfdr.de>; Wed, 20 May 2020 09:06:35 +0200 (CEST)
Received: (majordomo@vger.kernel.org) by vger.kernel.org via listexpand
        id S1726966AbgETHE1 (ORCPT <rfc822;lists+linux-rdma@lfdr.de>);
        Wed, 20 May 2020 03:04:27 -0400
Received: from mga01.intel.com ([192.55.52.88]:53512 "EHLO mga01.intel.com"
        rhost-flags-OK-OK-OK-OK) by vger.kernel.org with ESMTP
        id S1726905AbgETHEW (ORCPT <rfc822;linux-rdma@vger.kernel.org>);
        Wed, 20 May 2020 03:04:22 -0400
IronPort-SDR: h2epnyQTHwB3tV5he3zfqyodAqDgvXRSeAa/BBAggP3awIsjPEm/+asjJl5fKHxoxikyhkgnkw
 zkDvAAzBrx6w==
X-Amp-Result: SKIPPED(no attachment in message)
X-Amp-File-Uploaded: False
Received: from orsmga003.jf.intel.com ([10.7.209.27])
  by fmsmga101.fm.intel.com with ESMTP/TLS/ECDHE-RSA-AES256-GCM-SHA384; 20 May 2020 00:04:19 -0700
IronPort-SDR: scROHAAoyBnIpVaSHFM7dyi7pWbBIjysTyirafGmkbIG5XAytbLxb3myXbYrVxN/T4F3v9b8Eq
 t0Zkd+qD5o9A==
X-ExtLoop1: 1
X-IronPort-AV: E=Sophos;i="5.73,413,1583222400"; 
   d="scan'208";a="264581249"
...
Subject: [RDMA RFC v6 12/16] RDMA/irdma: Add miscellaneous utility definitions
Date:   Wed, 20 May 2020 00:04:11 -0700
Message-Id: <20200520070415.3392210-13-jeffrey.t.kirsher@intel.com>
...
---
 ...
 4 files changed, 2724 insertions(+)
...
index 000000000000..e08343673ac2
--- /dev/null
...
+#define idev_to_dev(ptr) (&((ptr)->hw->pdev->dev))
+#define ihw_to_dev(hw)   (&(hw)->pdev->dev)
...
new file mode 100644
index 000000000000..ebc1c9b8cfc2
--- /dev/null
...
new file mode 100644
index 000000000000..d5108872a3c7
--- /dev/null
...
+		status = 0;
+	} else if (status) {
+		irdma_dbg(&rf->sc_dev, "ERR: error cqp command 0x%x failed\n",
+			  info->cqp_cmd);
+		irdma_free_cqp_request(&rf->cqp, cqp_request);
+		return status;
+	}
...
+	if (status)
+		irdma_dbg(dev, "ERR: CQP-OP Update SD's fail");
+
+	return status;
+}
+
...
+	status = irdma_handle_cqp_op(rf, cqp_request);
+	if (status)
+		irdma_dbg(dev, "ERR: CQP-OP QP Suspend/Resume fail");
+
+	return status;
+}
+
...
+	status = irdma_handle_cqp_op(rf, cqp_request);
+	if (status)
+		irdma_dbg(dev, "ERR: CQP-OP Query FPM fail");
+
+	return status;
+}
+
...
+	status = irdma_handle_cqp_op(rf, cqp_request);
+	if (status)
+		irdma_dbg(dev, "ERR: CQP-OP Commit FPM fail");
+
+	return status;
+}
+
...
+	status = irdma_handle_cqp_op(rf, cqp_request);
+	if (status)
+		irdma_dbg(dev, "ERR: CQP-OP Create CQ fail");
+
+	return status;
+}
+
...
+	status = irdma_handle_cqp_op(rf, cqp_request);
+	if (status)
+		irdma_dbg(dev, "ERR: CQP-OP QP create fail");
+
+	return status;
+}
+
...
+	else
+		irdma_dbg(&rf->sc_dev, "ERR: CQP-OP dealloc Push page fail");
+}
+
+/**
...
+	status = irdma_handle_cqp_op(rf, cqp_request);
+	if (status)
+		irdma_dbg(&rf->sc_dev, "ERR: CQP-OP Destroy CQ fail");
+}
+
+/**
...
+	status = irdma_handle_cqp_op(rf, cqp_request);
+	if (status)
+		irdma_dbg(dev, "ERR: CQP QP_DESTROY fail");
+}
+
+/**
...
+		ibdev_warn(&iwdev->ibdev, "UDP RX in IEQ for qp_id %d\n", bth->qpn);
+		print_hex_dump_debug("IEQ: IEQ UDP RX BUFFER",
+				     DUMP_PREFIX_OFFSET, 16, 8, buf->mem.va,
+				     buf->totallen, false);
+		return NULL;
+	}
+
...
+	status = irdma_handle_cqp_op(rf, cqp_request);
+	if (status)
+		irdma_dbg(dev, "ERR: CQP STATS_GATHER fail");
+	else if (wait)
+		irdma_process_stats(pestat);
+
+	return status;
...
+	status = irdma_handle_cqp_op(rf, cqp_request);
+	if (status)
+		irdma_dbg(&rf->sc_dev, "ERR: CQP MANAGE_STATS fail");
+	else if (wait)
...
+	status = irdma_handle_cqp_op(rf, cqp_request);
+	if (status) {
+		irdma_dbg(dev, "ERR: CQP WS_NODE fail\n");
+		return status;
+	}
+
+	if (poll) {
...
+	status = irdma_handle_cqp_op(rf, cqp_request);
+	if (status)
+		irdma_dbg(dev, "ERR: CQP UP MAP fail\n");
+
+	return status;
+}
+
...
+			goto err;
+
+		pg_arr[i] = dma_map_page(ihw_to_dev(hw), vm_page, 0,
+					 PAGE_SIZE, DMA_BIDIRECTIONAL);
+		if (dma_mapping_error(ihw_to_dev(hw), pg_arr[i]))
+			goto err;
+
+		addr += PAGE_SIZE;
+	}
...
+
+err:
+	irdma_unmap_vm_page_list(hw, pg_arr, i);
+	return IRDMA_ERR_NO_MEMORY;
+}
+
+void irdma_unmap_vm_page_list(struct irdma_hw *hw, u64 *pg_arr, u32 pg_cnt)
...
+
+	for (i = 0; i < pg_cnt; i++)
+		dma_unmap_page(ihw_to_dev(hw), pg_arr[i], PAGE_SIZE,
+			       DMA_BIDIRECTIONAL);
+}
+
+/**
