...
Received: from vger.kernel.org (vger.kernel.org [23.128.96.18])
	by mail.lfdr.de (Postfix) with ESMTP id 2B7AC3E944E
	for <lists+linux-rdma@lfdr.de>; Wed, 11 Aug 2021 17:13:01 +0200 (CEST)
Received: (majordomo@vger.kernel.org) by vger.kernel.org via listexpand
        id S232814AbhHKPNW (ORCPT <rfc822;lists+linux-rdma@lfdr.de>);
        Wed, 11 Aug 2021 11:13:22 -0400
Received: from smtp-fw-80006.amazon.com ([99.78.197.217]:8505 "EHLO
        smtp-fw-80006.amazon.com" rhost-flags-OK-OK-OK-OK) by vger.kernel.org
        with ESMTP id S232965AbhHKPNT (ORCPT
        <rfc822;linux-rdma@vger.kernel.org>); Wed, 11 Aug 2021 11:13:19 -0400
DKIM-Signature: v=1; a=rsa-sha256; c=relaxed/relaxed;
  d=amazon.com; i=@amazon.com; q=dns/txt; s=amazon201209;
  t=1628694776; x=1660230776;
  h=from:to:cc:subject:date:message-id:in-reply-to:
   references:mime-version:content-transfer-encoding;
  bh=68mr5wZOwMhmCcTHBA0eYDs/Nan4Mesc22DKsd7T+Y8=;
  b=dtRKh2UtZUy7oqMiNNN0LlGx72r5BBo91STOZVEiIxnok2XhPPR+p7LF
   vla09RKIystgBiDRqP7nquXll2nkkdK9nkoZUgQXQ3iQedSePUfWrgCsi
   Zu/ske6P0r0LmID20jB5JuRNKwZLJfhtHqRg2bXaJIanlqqPnHS88EDOG
   o=;
X-IronPort-AV: E=Sophos;i="5.84,313,1620691200"; 
   d="scan'208";a="18583024"
Received: from pdx4-co-svc-p1-lb2-vlan3.amazon.com (HELO email-inbound-relay-2a-6e2fc477.us-west-2.amazon.com) ([10.25.36.214])
  by smtp-border-fw-80006.pdx80.corp.amazon.com with ESMTP; 11 Aug 2021 15:12:48 +0000
Received: from EX13D19EUB001.ant.amazon.com (pdx1-ws-svc-p6-lb9-vlan2.pdx.amazon.com [10.236.137.194])
        by email-inbound-relay-2a-6e2fc477.us-west-2.amazon.com (Postfix) with ESMTPS id CC09DA05D3;
        Wed, 11 Aug 2021 15:12:47 +0000 (UTC)
Received: from EX13MTAUEA002.ant.amazon.com (10.43.61.77) by
 EX13D19EUB001.ant.amazon.com (10.43.166.229) with Microsoft SMTP Server (TLS)
 id 15.0.1497.23; Wed, 11 Aug 2021 15:12:46 +0000
Received: from 8c85908914bf.ant.amazon.com.com (10.1.212.21) by
 ...
 15.0.1497.23 via Frontend Transport; Wed, 11 Aug 2021 15:12:43 +0000
...
Subject: [PATCH for-next 4/4] RDMA/efa: CQ notifications
Date:   Wed, 11 Aug 2021 18:11:31 +0300
Message-ID: <20210811151131.39138-5-galpress@amazon.com>
...
---
 drivers/infiniband/hw/efa/efa.h               |  16 +-
...
index 87b1dadeb7fe..e72bc4741ca1 100644
...
@@ -476,7 +607,7 @@ static struct efa_dev *efa_probe_device(struct pci_dev *pdev)
 	return dev;
...
@@ -499,11 +630,12 @@ static void efa_remove_device(struct pci_dev *pdev)
...
index e5f9d90aad5e..a5d1ff63bc7c 100644
...
@@ -993,15 +1002,22 @@ int efa_destroy_cq(struct ib_cq *ibcq, struct ib_udata *udata)
...
+static int efa_vec2eqn(struct efa_dev *dev, int vec)
+{
+	return dev->eqs[vec].eeq.eqn;
+}
+
...
@@ -1011,6 +1027,21 @@ static int cq_mmap_entries_setup(struct efa_dev *dev, struct efa_cq *cq,
 	if (!cq->mmap_entry)
 		return -ENOMEM;
...
@@ -1019,8 +1050,8 @@ int efa_create_cq(struct ib_cq *ibcq, const struct ib_cq_init_attr *attr,
 {
...
@@ -1065,7 +1096,7 @@ int efa_create_cq(struct ib_cq *ibcq, const struct ib_cq_init_attr *attr,
 		goto err_out;
 	}
...
+	if (cmd.comp_mask || !is_reserved_cleared(cmd.reserved_58)) {
 		...
 		err = -EINVAL;
@@ -1101,29 +1132,42 @@ int efa_create_cq(struct ib_cq *ibcq, const struct ib_cq_init_attr *attr,
 	params.dma_addr = cq->dma_addr;
 	params.entry_size_in_bytes = cmd.cq_entry_size;
 	params.num_sub_cqs = cmd.num_sub_cqs;
+	if (cmd.flags & EFA_CREATE_CQ_WITH_COMPLETION_CHANNEL) {
+		params.eqn = efa_vec2eqn(dev, attr->comp_vector);
...
@@ -1132,8 +1176,10 @@ int efa_create_cq(struct ib_cq *ibcq, const struct ib_cq_init_attr *attr,
