...
Received: from vger.kernel.org (vger.kernel.org [23.128.96.18])
	by mail.lfdr.de (Postfix) with ESMTP id 929C4301141
	for <lists+linux-rdma@lfdr.de>; Sat, 23 Jan 2021 00:59:01 +0100 (CET)
Received: (majordomo@vger.kernel.org) by vger.kernel.org via listexpand
        id S1726868AbhAVX5Q (ORCPT <rfc822;lists+linux-rdma@lfdr.de>);
        Fri, 22 Jan 2021 18:57:16 -0500
Received: from mga05.intel.com ([192.55.52.43]:55257 "EHLO mga05.intel.com"
        rhost-flags-OK-OK-OK-OK) by vger.kernel.org with ESMTP
        id S1726158AbhAVXzE (ORCPT <rfc822;linux-rdma@vger.kernel.org>);
        Fri, 22 Jan 2021 18:55:04 -0500
IronPort-SDR: GzQzj1LSAnVWEXnl7sP+ywuFn8N+AcaHoStdDpP1HR/RAmx02VAbDkfvOPv2M4MYoMhVlTngWZ
 NVzHGMxkPOJw==
X-IronPort-AV: E=McAfee;i="6000,8403,9872"; a="264346900"
X-IronPort-AV: E=Sophos;i="5.79,368,1602572400"; 
   d="scan'208";a="264346900"
Received: from fmsmga005.fm.intel.com ([10.253.24.32])
  by fmsmga105.fm.intel.com with ESMTP/TLS/ECDHE-RSA-AES256-GCM-SHA384; 22 Jan 2021 15:50:26 -0800
IronPort-SDR: ysXwbbpGcKJe8KLpq0zurg7+Stmil1cCd3N6s3MDAqtpKJWj1BSQvnIj8w/CXDJmEuvMNmIUIk
 tGMCo1ZsvlKQ==
X-IronPort-AV: E=Sophos;i="5.79,368,1602572400"; 
   d="scan'208";a="574869567"
Received: from ssaleem-mobl.amr.corp.intel.com ([10.251.4.95])
  by fmsmga005-auth.fm.intel.com with ESMTP/TLS/ECDHE-RSA-AES256-GCM-SHA384; 22 Jan 2021 15:50:24 -0800
...
Subject: [PATCH 15/22] RDMA/irdma: Implement device supported verb APIs
Date:   Fri, 22 Jan 2021 17:48:20 -0600
Message-Id: <20210122234827.1353-16-shiraz.saleem@intel.com>
...
+	if (!rdma_entry) {
+		irdma_dbg(iwdev_to_idev(ucontext->iwdev),
...
+	u64 rsvd, bar_off;
+
+	rsvd = (iwdev->rf->priv_peer_info.ftype ? IRDMA_VF_BAR_RSVD : IRDMA_PF_BAR_RSVD);
...
+	mem->va = dma_alloc_coherent(ihw_to_dev(iwdev->rf->sc_dev.hw),
...
+				      256);
+	iwqp->q2_ctx_mem.va = dma_alloc_coherent(ihw_to_dev(dev->hw),
...
+
+	if (err_code) {
+		irdma_dbg(iwdev_to_idev(iwdev), "VERBS: setup qp failed\n");
+		goto error;
+	}
+
+	if (rdma_protocol_roce(&iwdev->ibdev, 1)) {
...
+	if (ret) {
+		err_code = -EPROTO;
+		irdma_dbg(iwdev_to_idev(iwdev), "VERBS: qp_init fail\n");
+		goto error;
+	}
+
...
+			uresp_gen1.push_idx = IRDMA_INVALID_PUSH_PAGE_INDEX_GEN_1;
+			uresp_gen1.lsmm = 1;
...
+		     iwdev->vsi.mtu)) {
+			ibdev_warn(&iwdev->ibdev, "Invalid RoCE MTU %d, Device MTU %d\n",
+				   attr->path_mtu, iwdev->vsi.mtu);
+			return -EINVAL;
+		}
+
...
+
+	if (!iwcq->user_mode) {
+		dma_free_coherent(ihw_to_dev(rf->sc_dev.hw), iwcq->kmem.size,
+				  iwcq->kmem.va, iwcq->kmem.pa);
+		iwcq->kmem.va = NULL;
+		dma_free_coherent(ihw_to_dev(rf->sc_dev.hw),
...
+
+	dma_free_coherent(ihw_to_dev(cq_buf->hw), cq_buf->kmem_buf.size,
...
+		kmem_buf.va = dma_alloc_coherent(ihw_to_dev(dev->hw),
...
+error:
+	if (!udata) {
+		dma_free_coherent(ihw_to_dev(dev->hw), kmem_buf.size,
...
+		iwcq->kmem.va = dma_alloc_coherent(ihw_to_dev(dev->hw),
+						   iwcq->kmem.size,
...
+					       64);
+		iwcq->kmem_shadow.va = dma_alloc_coherent(ihw_to_dev(dev->hw),
+							  iwcq->kmem_shadow.size,
+							  &iwcq->kmem_shadow.pa,
+							  GFP_KERNEL);
...
+		irdma_dbg(iwdev_to_idev(iwdev), "VERBS: init cq fail\n");
+		err_code = -EPROTO;
+		goto cq_free_rsrc;
+	}
+
...
+	break;
+	default:
+		irdma_dbg(iwdev_to_idev(iwdev), "VERBS: MR type error\n");
+		err = -EINVAL;
+	}
+
+	if (use_pbles && ret) {
...
+		default:
+			err = -EINVAL;
+			irdma_dbg(iwdev_to_idev(iwqp->iwdev),
...
+}
+
+static int irdma_roce_port_immutable(struct ib_device *ibdev, u8 port_num,
...
+}
+
+static int irdma_iw_port_immutable(struct ib_device *ibdev, u8 port_num,
+				   struct ib_port_immutable *immutable)
+{
+	struct ib_port_attr attr;
+	int err;
...
+					 IRDMA_HW_PAGE_SIZE);
+		dma_mem_mc->va = dma_alloc_coherent(ihw_to_dev(&rf->hw),
+						    dma_mem_mc->size,
+						    &dma_mem_mc->pa,
+						    GFP_KERNEL);
+		if (!dma_mem_mc->va) {
...
+		dma_free_coherent(ihw_to_dev(&rf->hw),
...
+{
+	struct pci_dev *pcidev = iwdev->rf->hw.pcidev;
+	int ret;
+
+	if (iwdev->roce_mode) {
...
+	if (ret)
+		goto error;
+	dma_set_max_seg_size(&iwdev->rf->hw.pcidev->dev, UINT_MAX);
+	ret = ib_register_device(&iwdev->ibdev, "irdma%d", &iwdev->rf->hw.pcidev->dev);
+	if (ret)
+		goto error;
+
+	iwdev->iw_status = 1;
