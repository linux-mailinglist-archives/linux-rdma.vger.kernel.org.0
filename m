...
Received: from out1.vger.email (out1.vger.email [IPv6:2620:137:e000::1:20])
	by mail.lfdr.de (Postfix) with ESMTP id 053995325D3
	for <lists+linux-rdma@lfdr.de>; Tue, 24 May 2022 10:56:54 +0200 (CEST)
Received: (majordomo@vger.kernel.org) by vger.kernel.org via listexpand
        id S234014AbiEXI4t (ORCPT <rfc822;lists+linux-rdma@lfdr.de>);
        Tue, 24 May 2022 04:56:49 -0400
Received: from lindbergh.monkeyblade.net ([23.128.96.19]:60398 "EHLO
        lindbergh.monkeyblade.net" rhost-flags-OK-OK-OK-OK) by vger.kernel.org
        with ESMTP id S234000AbiEXI4j (ORCPT
        <rfc822;linux-rdma@vger.kernel.org>); Tue, 24 May 2022 04:56:39 -0400
Received: from linux.microsoft.com (linux.microsoft.com [13.77.154.182])
        by lindbergh.monkeyblade.net (Postfix) with ESMTP id 7D53C72E17;
        Tue, 24 May 2022 01:56:35 -0700 (PDT)
Received: by linux.microsoft.com (Postfix, from userid 1004)
        id EF91520B894E; Tue, 24 May 2022 01:56:34 -0700 (PDT)
DKIM-Filter: OpenDKIM Filter v2.11.0 linux.microsoft.com EF91520B894E
DKIM-Signature: v=1; a=rsa-sha256; c=relaxed/relaxed; d=linuxonhyperv.com;
        s=default; t=1653382594;
        bh=tINg1EzRPKfy50nrDvG1LYPzip2orKhQ9sQ7RAZ+9GM=;
        h=From:To:Cc:Subject:Date:In-Reply-To:References:Reply-To:From;
        b=S2jT+fhIoeRfOKsiMVM8i8NtJA7K7/zvcjpHaNrT9kSI784hkcVrwSji8Big9oOXT
         vl7fXtksVgerotnf3Ya/3jMDbDt+OpFUsTjeXKPauzPKZWiN40unoFxkhUl4MwE7iW
         rUW1IbTwmZV3ydxGjijwx0X0Mqo2D6WSjYfq0T0c=
...
Subject: [Patch v2 12/12] RDMA/mana_ib: Add a driver for Microsoft Azure Network Adapter
Date:   Tue, 24 May 2022 01:56:12 -0700
Message-Id: <1653382572-14788-13-git-send-email-longli@linuxonhyperv.com>
...
Fixed warnings reported by kernel test robot <lkp@intel.com>
 ...
 14 files changed, 1702 insertions(+)
...
new file mode 100644
index 000000000000..8707a4d52668
--- /dev/null
...
new file mode 100644
index 000000000000..6b02f4eb5ead
--- /dev/null
...
+
+	if (refcount_dec_and_test(&pd->vport_use_count))
...
+	int err;
+
+	refcount_inc(&pd->vport_use_count);
+	if (refcount_read(&pd->vport_use_count) > 1) {
...
+	if (err) {
+		ibdev_err(&dev->ib_dev, "mana_cfg_vport err %d\n", err);
+		refcount_dec(&pd->vport_use_count);
+		return err;
+	}
+
...
+		dma_addr_t cur_addr = rdma_block_iter_dma_address(&biter);
+
...
new file mode 100644
index 000000000000..7fec826d9d9d
--- /dev/null
...
new file mode 100644
index 000000000000..f59b0b4b3aad
--- /dev/null
...
+		ibdev_err(
+			ibdev,
+			"Failed to create dma region for register user-mr, %d\n",
+			err);
+		goto err_umem;
+	}
+
...
new file mode 100644
index 000000000000..df190cf9c113
--- /dev/null
...
+		ibdev_dbg(
+			&mdev->ib_dev,
+			"Failed to copy from udata for create rss-qp, err %d\n",
+			ret);
+		return -EFAULT;
+	}
+
...
+	if (err) {
+		ibdev_err(&mdev->ib_dev, "cfg vport failed err %d\n", err);
+		return -ENODEV;
+	}
+
+	qp->port = port;
...
+			ibdev_dbg(
+				&mdev->ib_dev,
+				"Failed to copy udata for create qp-raw, %d\n",
+				err);
...
new file mode 100644
index 000000000000..e23f633d64a5
--- /dev/null
...
+	struct mana_ib_dev *mdev =
+		container_of(ib_dev, struct mana_ib_dev, ib_dev);
+
...
index a58e86b0aebe..09c382d1a6d8 100644
...
new file mode 100644
index 000000000000..2fabc56282f1
--- /dev/null
