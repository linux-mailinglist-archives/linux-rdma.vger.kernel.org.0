...
Received: from out1.vger.email (out1.vger.email [IPv6:2620:137:e000::1:20])
	by mail.lfdr.de (Postfix) with ESMTP id D33624BB79C
	for <lists+linux-rdma@lfdr.de>; Fri, 18 Feb 2022 12:05:42 +0100 (CET)
Received: (majordomo@vger.kernel.org) by vger.kernel.org via listexpand
        id S233223AbiBRLF5 (ORCPT <rfc822;lists+linux-rdma@lfdr.de>);
        Fri, 18 Feb 2022 06:05:57 -0500
Received: from mxb-00190b01.gslb.pphosted.com ([23.128.96.19]:45758 "EHLO
        lindbergh.monkeyblade.net" rhost-flags-OK-OK-OK-OK) by vger.kernel.org
        with ESMTP id S234248AbiBRLFl (ORCPT
        <rfc822;linux-rdma@vger.kernel.org>); Fri, 18 Feb 2022 06:05:41 -0500
Received: from szxga03-in.huawei.com (szxga03-in.huawei.com [45.249.212.189])
        by lindbergh.monkeyblade.net (Postfix) with ESMTPS id DFBDB28881F
        for <linux-rdma@vger.kernel.org>; Fri, 18 Feb 2022 03:05:23 -0800 (PST)
Received: from dggpeml500022.china.huawei.com (unknown [172.30.72.54])
        by szxga03-in.huawei.com (SkyGuard) with ESMTP id 4K0TKh0rVDz8wWB;
...
Subject: [PATCH for-next 5/8] RDMA/hns: Refactor mailbox functions
Date:   Fri, 18 Feb 2022 19:05:16 +0800
Message-ID: <20220218110519.37375-6-liangwenpeng@huawei.com>
...
+	hns_roce_set_poll_mbox_msg(mbox_msg);
...
+	ret = __hns_roce_cmd_mbox_poll(hr_dev, mbox_msg);
 	...
 	return ret;
@@ -97,9 +115,8 @@ void hns_roce_cmd_event(struct hns_roce_dev *hr_dev, u16 token, u8 status,
...
+				    struct hns_roce_mbox_msg *mbox_msg)
 {
 	...
 	struct hns_roce_cmd_context *context;
@@ -120,19 +137,19 @@ static int __hns_roce_cmd_mbox_wait(struct hns_roce_dev *hr_dev, u64 in_param,
 
 	reinit_completion(&context->done);
 
-	ret = hns_roce_cmd_mbox_post_hw(hr_dev, in_param, out_param,
-					in_modifier, op, context->token, 1);
+	hns_roce_set_event_mbox_msg(mbox_msg, context);
+	ret = hns_roce_cmd_mbox_post_hw(hr_dev, mbox_msg);
 	if (ret) {
...
@@ -140,42 +157,41 @@ static int __hns_roce_cmd_mbox_wait(struct hns_roce_dev *hr_dev, u64 in_param,
 	ret = context->result;
 	if (ret)
...
+	hns_roce_set_basic_mbox_msg(&mbox_msg, in_param, out_param, cmd, tag);
+
