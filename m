...
Received: from vger.kernel.org (vger.kernel.org [209.132.180.67])
	by mail.lfdr.de (Postfix) with ESMTP id 87CE822C65
	for <lists+linux-rdma@lfdr.de>; Mon, 20 May 2019 08:55:31 +0200 (CEST)
Received: (majordomo@vger.kernel.org) by vger.kernel.org via listexpand
        id S1730819AbfETGzb (ORCPT <rfc822;lists+linux-rdma@lfdr.de>);
        Mon, 20 May 2019 02:55:31 -0400
Received: from mail.kernel.org ([198.145.29.99]:39248 "EHLO mail.kernel.org"
        rhost-flags-OK-OK-OK-OK) by vger.kernel.org with ESMTP
        id S1730826AbfETGza (ORCPT <rfc822;linux-rdma@vger.kernel.org>);
        Mon, 20 May 2019 02:55:30 -0400
Received: from localhost (unknown [37.142.3.125])
        (using TLSv1.2 with cipher ECDHE-RSA-AES256-GCM-SHA384 (256/256 bits))
        (No client certificate requested)
        by mail.kernel.org (Postfix) with ESMTPSA id 40826208CA;
        Mon, 20 May 2019 06:55:26 +0000 (UTC)
DKIM-Signature: v=1; a=rsa-sha256; c=relaxed/simple; d=kernel.org;
        s=default; t=1558335327;
        bh=M/d+AyDwDS3O8zzkRr0gVn6TfFkhyMjNMB6/ZvAQFYI=;
        h=From:To:Cc:Subject:Date:In-Reply-To:References:From;
        b=oNfrcMJYr7Evh1n3VednFQQ9u7ZSCURrZH4huBDVLgafA9pCFi+o7noSYfB/HysDX
         QvZuOP+ZWGOAPiuLBqh9nmAZKd4xL7WO/Ep8qTu0J/UJpIe5qBZd9rIoaFGM1CidZ6
         WfiFK84gXaSEzkAvLI4OMeugCSGgGYviKLFEAt+A=
...
Subject: [PATCH rdma-next 15/15] RDMA: Convert CQ allocations to be under core responsibility
Date:   Mon, 20 May 2019 09:54:33 +0300
Message-Id: <20190520065433.8734-16-leon@kernel.org>
...
---
 ...
 48 files changed, 326 insertions(+), 444 deletions(-)

diff --git a/drivers/infiniband/core/cq.c b/drivers/infiniband/core/cq.c
index de9fd7089349..d3bb21055afa 100644
...
index ca1dba5f9ec6..a67aaf0e1f76 100644
--- a/drivers/infiniband/core/device.c
+++ b/drivers/infiniband/core/device.c
@@ -2438,6 +2438,7 @@ void ib_set_device_ops(struct ib_device *dev, const struct ib_device_ops *ops)
...
index 78103a95943e..3c62489fea46 100644
...
index 168c05b3e9e5..812d93fe2bd1 100644
...
@@ -200,13 +192,13 @@ static struct ib_cq *iwch_create_cq(struct ib_device *ibdev,
...
@@ -1272,6 +1264,7 @@ static const struct ib_device_ops iwch_dev_ops = {
...
index 2319d0f983b0..7f742c2f5e6b 100644
...
-
 	...
 	if (!chp->wr_waitp) {
 		ret = -ENOMEM;
@@ -1133,10 +1126,11 @@ struct ib_cq *c4iw_create_cq(struct ib_device *ibdev,
 		mm2->len = PAGE_SIZE;
...
+	return 0;
 err_free_mm2:
 	kfree(mm2);
 err_free_mm:
@@ -1152,8 +1146,7 @@ struct ib_cq *c4iw_create_cq(struct ib_device *ibdev,
 err_free_wr_wait:
 	c4iw_put_wr_wait(chp->wr_waitp);
 err_free_chp:
-	kfree(chp);
...
index 7e014c33c718..5abd1d9313b8 100644
...
index a6936aa0fde6..356d373accc1 100644
--- a/drivers/infiniband/hw/hns/hns_roce_device.h
+++ b/drivers/infiniband/hw/hns/hns_roce_device.h
@@ -1178,9 +1178,9 @@ void hns_roce_release_range_qp(struct hns_roce_dev *hr_dev, int base_qpn,
...
index 955f36352d20..258f9ade53a4 100644
...
@@ -3689,8 +3696,6 @@ static void hns_roce_v1_destroy_cq(struct ib_cq *ibcq, struct ib_udata *udata)
...
index 8da5f18bf820..d93fe62b2b4d 100644
--- a/drivers/infiniband/hw/hns/hns_roce_main.c
+++ b/drivers/infiniband/hw/hns/hns_roce_main.c
@@ -451,6 +451,7 @@ static const struct ib_device_ops hns_roce_dev_ops = {
...
index d2a230d6c0e7..e3081c8c31a3 100644
--- a/drivers/infiniband/hw/qedr/verbs.c
+++ b/drivers/infiniband/hw/qedr/verbs.c
@@ -813,20 +813,20 @@ int qedr_arm_cq(struct ib_cq *ibcq, enum ib_cq_notify_flags flags)
 	return 0;
 }
...
+	struct qedr_cq *cq = get_qedr_cq(ibcq);
 	...
 	int page_cnt;
 	u64 pbl_ptr;
@@ -841,18 +841,13 @@ struct ib_cq *qedr_create_cq(struct ib_device *ibdev,
 		DP_ERR(dev,
...
@@ -930,7 +925,7 @@ struct ib_cq *qedr_create_cq(struct ib_device *ibdev,
...
@@ -945,8 +940,7 @@ struct ib_cq *qedr_create_cq(struct ib_device *ibdev,
 	if (udata)
...
@@ -976,7 +970,7 @@ void qedr_destroy_cq(struct ib_cq *ibcq, struct ib_udata *udata)
...
@@ -1015,10 +1009,6 @@ void qedr_destroy_cq(struct ib_cq *ibcq, struct ib_udata *udata)
...
index b971effecad9..e9fffa55426b 100644
